...
function reorder!(
    ...
    @boundscheck out.grid == field.grid ||
                 throw(BoundsError("Reordering requires identical grids, got $(out.grid) and $(field.grid)."))
    ...
    launch!(
        arch, RingGridWorkOrder, worksize, reorder_kernel!, out, field, order, field.grid)
    return out
end

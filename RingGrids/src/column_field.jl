...
function transpose_safe(field::Field2D)
    return ColumnField(collect(transpose(field.data)), field.grid)
end
...
_data_transpose!(data::AbstractVector, scratch::AbstractArray) = reshape(data, _size_of_transpose(data))

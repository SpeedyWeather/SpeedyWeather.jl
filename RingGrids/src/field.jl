...
$(TYPEDFIELDS)"""
struct Field{T, N, ArrayType <: AbstractArray, Grid <: AbstractGrid} <:
       AbstractField{T, N, ArrayType, Grid}
    data::ArrayType
    grid::Grid

    # Inner constructor to check for matching grid and data
    function Field(data, grid)
        data_matches_grid(data, grid; horizontal_dim = 1) ||
            throw(DimensionMismatch(data, grid))
...
function (::Type{<:Field{T}})(data::AbstractArray, grid::AbstractGrid) where {T}
    Field(T.(data), grid)
end
...
function Architectures.ismatching(arch::AbstractArchitecture, field::AbstractField)
    ismatching(arch, field.data)
end

# test number of horizontal grid points matches
function data_matches_grid(data::AbstractArray, grid::AbstractGrid; horizontal_dim::Integer = 1)
    size(data, horizontal_dim) == get_npoints(grid)
end
...
function Base.DimensionMismatch(f1::AbstractField, f2::AbstractField)
    DimensionMismatch("$(summary(f1)) does not match $(summary(f2))")
end
function Base.DimensionMismatch(f1::AbstractField, f2::AbstractArray)
    DimensionMismatch("$(summary(f1)) does not match $(summary(f2))")
end
function Base.DimensionMismatch(f1::AbstractField, f2s::AbstractField...)
    length(f2s) == 0 &&
        return DimensionMismatch("$(summary(f1)) does not match any other field")
...
function eachring(field1::AbstractField, fields::AbstractField...; horizontal_only = true, kwargs...)
    fields_match(field1, fields...; horizontal_only, kwargs...) ||
        throw(DimensionMismatch(field1, fields...))
...
function eachlayer(field1::AbstractField, fields::AbstractField...; vertical_only = true, kwargs...)
    fields_match(field1, fields...; vertical_only, kwargs...) ||
        throw(DimensionMismatch(field1, fields...))
...
function (::Type{F})(data::AbstractArray; input_as = Vector) where {F <: AbstractField}
    F(data, input_as)
end
...
function (::Type{F})(data::AbstractArray, input_as::Type{Matrix}) where {F <:
...
function Base.Array(field::AbstractFullField, as::Type{Matrix})
    Array(reshape(field.data, :, get_nlat(field), size(field)[2:end]...))
end
function Base.Array(field::AbstractReducedField, as::Type{Matrix})
    error("$(typeof(field)), i.e. data on a reduced grid, cannot be converted to a Matrix.")
end
...
for f in (:zeros, :ones, :rand, :randn)
    @eval begin
        ...
        function Base.$f(Grid::Type{<:AbstractGrid}, nlat_half::Integer,
                k::Integer...; architecture = DEFAULT_ARCHITECTURE())
...
function Base.similar(field::F, ::Type{T}) where {F <: AbstractField, T}
    F(similar(field.data, T), field.grid)
end
...
function (::Type{F})(
        ...
        ArrayType,
        Grid}} where {
        T, N, ArrayType, Grid <: AbstractGrid{Architecture}} where {Architecture}
...
function (::Type{F})(
        ...
        T, N, ArrayType, Grid}} where {T, N, ArrayType, Grid <: AbstractGrid}
...
function Base.:(==)(F1::AbstractField, F2::AbstractField)
    fields_match(F1, F2) && F1.data == F2.data
end
Base.all(F::AbstractField) = all(F.data)
Base.any(F::AbstractField) = any(F.data)

function grids_match(A::AbstractField, Bs::AbstractField...)
    grids_match(A.grid, (B.grid for B in Bs)...)
end
...
function field_view(field::AbstractField, c::Colon, i, args...)
    Field(view(field.data, c, i, args...), field.grid)
end
...
function Base.BroadcastStyle(::Type{F}) where {F <: Field{
        T, N, ArrayType, Grid}} where {T, N, ArrayType, Grid}
...
function Base.similar(bc::Broadcasted{FieldStyle{N, Grid, false}}, ::Type{T}) where {
        N, Grid, T}
    field = find_field(bc)
...
function FieldStyle{N, Grid, Column}(::Val{N}) where {N, Grid, Column}
    FieldStyle{N, Grid, Column}()
end
...
function Base.BroadcastStyle(
        ...
        T, N, ArrayType, Grid}} where {T, N, ArrayType <: GPUArrays.AbstractGPUArray, Grid}
    return FieldGPUStyle{N, Grid, false}()
end

function Base.similar(bc::Broadcasted{FieldGPUStyle{N, Grid, false}}, ::Type{T}) where {
        N, Grid, T}
    field = find_field(bc)
    ArrayType_ = nonparametric_type(typeof(field.data))
    new_data = ArrayType_{T}(undef, size(bc))
...
# ::Val{0} for broadcasting with 0-dimensional, ::Val{1} for broadcasting with vectors, etc
# when there's a dimension mismatch always choose the larger dimension
function FieldGPUStyle{N, Grid, Column}(::Val{N}) where {N, Grid, Column}
    FieldGPUStyle{N, Grid, Column}()
end
function FieldGPUStyle{1, Grid, Column}(::Val{2}) where {Grid, Column}
    FieldGPUStyle{2, Grid, Column}()
end
function FieldGPUStyle{1, Grid, Column}(::Val{0}) where {Grid, Column}
    FieldGPUStyle{1, Grid, Column}()
end
function FieldGPUStyle{2, Grid, Column}(::Val{3}) where {Grid, Column}
    FieldGPUStyle{3, Grid, Column}()
end
function FieldGPUStyle{2, Grid, Column}(::Val{1}) where {Grid, Column}
    FieldGPUStyle{2, Grid, Column}()
end
function FieldGPUStyle{3, Grid, Column}(::Val{4}) where {Grid, Column}
    FieldGPUStyle{4, Grid, Column}()
end
function FieldGPUStyle{3, Grid, Column}(::Val{2}) where {Grid, Column}
    FieldGPUStyle{2, Grid, Column}()
end
...
function KernelAbstractions.get_backend(
        ...
        T, N, ArrayType}} where {T, N, ArrayType <: GPUArrays.AbstractGPUArray}
...
function Architectures.on_architecture(arch, field::Field{
        T, N, ArrayType, Grid}) where {T, N, ArrayType, Grid}

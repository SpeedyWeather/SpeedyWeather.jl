...
function update_locator!(
    ...
    return find_grid_indices!(locator, geometry, λs, geometry.grid.architecture)               # next points left and right of λ on rings north and south
end
...
function update_locator!(
    ...
    return update_locator!(locator, geometry, londs, latds; kwargs...)
end
...
function find_rings!(
    ...
    if ~unsafe
        ...
        @boundscheck θmin >= -90 "Latitudes θs are expected to be within [-90˚, 90˚]; θ=$(θmin)˚ given."
        @boundscheck θmax <= 90 "Latitudes θs are expected to be within [-90˚, 90˚]; θ=$(θmax)˚ given."

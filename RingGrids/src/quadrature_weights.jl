...
`sum(gaussian_weights(nlat_half))` is always `2` as ``∫_0^π \\sin x dx = 2`` (colatitudes),
...
`sum(clenshaw_curtis_weights(nlat_half))` is always `2` as ``∫_0^π \\sin x dx = 2`` (colatitudes),
or equivalently ``∫_{-π/2}^{π/2} \\cos x dx`` (latitudes).

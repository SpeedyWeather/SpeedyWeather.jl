...
write(md,
    "Benchmarking excludes initialization and is started just before the main time loop and finishes right after. ")
write(md,
    "The benchmarking results here are not very robust, timings that change with +-50% are not uncommon. ")
write(md,
    "Proper benchmarking for performance optimization uses the minimum or median of many executions, ")
write(md, "while we run a simulation for several time steps which effectively represents the mean, ")
write(md,
    "susceptible to outliers that slow down the simulation. However, this is what a user will experience ")
...
write(md,
    "- T: Spectral resolution, maximum degree of spherical harmonics, default: T$(SpeedyWeather.DEFAULT_TRUNC)\n")
...
write(md,
    "inside `the SpeedyWeather.jl/benchmark` folder. It will create this `README.md` which can be pushed ")

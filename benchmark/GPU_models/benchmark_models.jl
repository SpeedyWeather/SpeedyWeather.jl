...
function run_benchmark(simulation; ntrials = 1, nsteps = 1500)
    ...
    for i in 1:ntrials
        ...
        time_elapsed = simulation.model.feedback.progress_meter.tlast -
                       simulation.model.feedback.progress_meter.tinit
...
function benchmark_cpu_vs_gpu(truncations::Vector{Int}; nlayers::Int = 1,
        ntrials::Int = 1, nsteps::Union{Int, Vector{Int}} = 1000)
    cpu_times = Float64[]
    gpu_times = Float64[]
...
function plot_results(truncations, cpu_times, gpu_times; filename = "cpu_vs_gpu_benchmark.pdf")
    ...
    annotate!(p[2], [(
        truncations[1], minimum(speedups), ("Generated: $timestamp", 8, :left, :bottom))])

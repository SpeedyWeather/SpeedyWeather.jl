@testset "Output on various grids" begin
    ...
    p = run_speedy(Float64, Grid = OctahedralClenshawGrid, output_grid = :matrix,
                   output = true)
    @test all(isfinite.(p.layers[1].leapfrog[1].vor))

    p = run_speedy(Float64, Grid = OctahedralClenshawGrid, output_grid = :matrix,
                   output_NF = Float32, output = true)
    @test all(isfinite.(p.layers[1].leapfrog[1].vor))
end
...
@testset "Restart from output file" begin
    ...
    progn, diagn, model = initialize_speedy(Float32, initial_conditions = :restart,
                                            model = ShallowWaterModel,
                                            restart_id = "restart-test")

    for varname in propertynames(progn.layers[1].leapfrog[1])
        if SpeedyWeather.has(progn, varname)
            for (var_new, var_old) in zip(SpeedyWeather.get_var(p, varname),
                                          SpeedyWeather.get_var(progn, varname))

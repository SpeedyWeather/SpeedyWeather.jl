...
@testset "Leapfrog oscillation" begin
    ...
    for NF in (Float16sr,Float16,Float32,Float64,Posit16,Posit32)
        P = Parameters(NF=NF)
        C = Constants(P)
        ...
        error = abs(Xout[end]) #- Real{NF}(1.0)
        println("Magnitude after " * string(n_timesteps) * " timesteps for NF " * string(NF) * " is: ", error)

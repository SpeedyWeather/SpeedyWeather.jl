...
@testset "SpeedyParam" begin
    ...
    p = SpeedyParam(42.0, bounds = RealLine(), desc = "Test parameter",
        unit = "m", category = "generic")
...
@testset "parameters" begin
    ...
    @test parameters(SpeedyParam, 1.0, bounds = UnitInterval()) ==
          SpeedyParam(1.0, bounds = UnitInterval())
    # test for non-nested type
    earth = Earth(Float32)
    ps = parameters(earth, category = "planet")
    pvals = (
        earth.radius, earth.rotation, earth.gravity, earth.axial_tilt, earth.solar_constant)
    ...
    @test vec(parameters(new_model2.atmosphere)).heat_capacity ==
          new_model_ps2.atmosphere.heat_capacity

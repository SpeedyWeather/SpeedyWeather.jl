@testset "Thermodynamics" begin
    @testset "get_thermodynamics!" begin @testset for NF in (Float32, Float64)
    ...
    @testset "interpolate!" begin @testset for NF in (Float32, Float64)
        _, _, model = SpeedyWeather.initialize_speedy(NF, model = PrimitiveEquation)
        (; nlev) = model.geometry
        ...
        @test all((A_full_level[2:nlev] .> A_half_level[1:(nlev - 1)] .>
                   A_full_level[1:(nlev - 1)])
                  .||
                  (A_full_level[2:nlev] .< A_half_level[1:(nlev - 1)] .<
                   A_full_level[1:(nlev - 1)]))
    end end

    @testset "Saturation vapour pressure" begin @testset for NF in (Float32, Float64)
    ...
    @testset "Saturation specific humidity" begin @testset for NF in (Float32, Float64)
    ...
    @testset "Dry static energy" begin @testset for NF in (Float32, Float64)
        _, _, model = SpeedyWeather.initialize_speedy(NF, model = PrimitiveEquation)
        (; nlev) = model.geometry
    ...
    @testset "Moist static energy" begin @testset for NF in (Float32, Float64)
        _, _, model = SpeedyWeather.initialize_speedy(NF, model = PrimitiveEquation)
        (; nlev) = model.geometry
    ...
    @testset "Saturation moist static energy" begin @testset for NF in (Float32, Float64)
        _, _, model = SpeedyWeather.initialize_speedy(NF, model = PrimitiveEquation)
        (; nlev) = model.geometry

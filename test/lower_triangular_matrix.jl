@testset "LowerTriangularMatrix" begin @testset for NF in (Float32, Float64)
    mmax = 32
...
@testset "LowerTriangularMatrix: fill, copy, randn, convert" begin @testset for NF in (Float32,
                                                                                       Float64)
    mmax = 32
    @testset for lmax in (mmax, mmax + 1)
        A = randn(Complex{NF}, lmax, mmax)
...
@testset "LowerTriangularMatrix: *,+,eachindex,similar" begin @testset for NF in (Float16,
                                                                                  Float32,
                                                                                  Float64)
...
@testset "LowerTriangularMatrix: copyto!" begin @testset for NF in (Float16, Float32,
                                                                    Float64)
...
@testset "LowerTriangularMatrix: broadcast" begin @testset for NF in (Float16, Float32,
                                                                      Float64)
    L1 = randn(LowerTriangularMatrix{NF}, 10, 10)
    L2 = copy(L1)

@testset "Divergence of a non-divergent flow zero?" begin @testset for NF in (Float32,
                                                                              Float64)
...
@testset "Curl of an irrotational flow zero?" begin @testset for NF in (Float32, Float64)
    p, d, m = initialize_speedy(NF,
                                model = ShallowWater)
...
@testset "Scale, unscale coslat" begin @testset for NF in (Float32, Float64)
...
@testset "Flipsign in divergence!, curl!" begin @testset for NF in (Float32, Float64)
    p, d, m = initialize_speedy(NF,
...
@testset "Add in divergence!, curl!" begin @testset for NF in (Float32, Float64)
    p, d, m = initialize_speedy(NF,
                                model = BarotropicModel)
...
@testset "D,ζ -> u,v -> D,ζ" begin @testset for NF in (Float32, Float64)
    p, d, m = initialize_speedy(NF,
                                model = ShallowWater)
...
@testset "∇×∇=0 and ∇⋅∇=∇²" begin for NF in (Float32, Float64)
    p, d, m = initialize_speedy(NF)

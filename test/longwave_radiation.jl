@testset "Parametrization: longwave radiation" begin
    @testset "radset!" begin @testset for NF in (Float32, Float64)
        _, diagn, model = SpeedyWeather.initialize_speedy(NF, model = PrimitiveEquation)

        # Just check the last band
        @test isapprox(model.parameterization_constants.fband[end, :],
                       [0.19498351, 0.12541235, 0.33106664, 0.2985375],
                       rtol = 0.1)
    end end
    @testset "radlw_down!" begin @testset for NF in (Float32, Float64)
        _, diagn, model = SpeedyWeather.initialize_speedy(NF, model = PrimitiveEquation,
                                                          nlev = 8)
        ...
        column = ColumnVariables{NF}(nlev = nlev, nband = nband,
                                     n_stratosphere_levels = n_stratosphere_levels)
        column.temp = fill(300.0, nlev)
    ...
    @testset "compute_bbe!" begin @testset for NF in (Float32, Float64)
        _, diagn, model = SpeedyWeather.initialize_speedy(NF, model = PrimitiveEquation,
                                                          nlev = 8)
    ...
    @testset "radlw_up!" begin @testset for NF in (Float32, Float64)
        _, diagn, model = SpeedyWeather.initialize_speedy(NF, model = PrimitiveEquation,
                                                          nlev = 8)
    ...
    @testset "longwave_radiation!" begin @testset for NF in (Float32, Float64)
        _, diagn, model = SpeedyWeather.initialize_speedy(NF, model = PrimitiveEquation,
                                                          nlev = 8)

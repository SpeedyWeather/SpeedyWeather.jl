@testset "Test PrognosticVariables set!" begin
    ...
    @test all(isapprox(
        prog_new.ocean.sea_ice_concentration, prog_old.ocean.sea_ice_concentration .+ C, atol = 1e-6))
    ...
    # numbers
    set!(simulation, vor = Float32(3.0), lf = lf)
    ...
    SpeedyTransforms.UV_from_vordiv!(U, V, get_step(prog_new.vor, lf),
        get_step(prog_new.div, lf), model.spectral_transform)

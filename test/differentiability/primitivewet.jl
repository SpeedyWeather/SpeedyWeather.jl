...
@testset "Differentiability: Primitive Wet Model Components" begin
    ...
    autodiff(Reverse, SpeedyWeather.parameterization_tendencies!, Const,
        Duplicated(diagn, ddiagn), Duplicated(progn, dprogn),
        Const(progn.clock.time), Const(model))
    ...
    fd_vjp = FiniteDifferences.j′vp(central_fdm(11, 1),
        x -> parameterization_tendencies(diagn_copy, x, progn.clock.time, model),
        ddiagn_copy, progn_copy)
    ...
    autodiff(Reverse, SpeedyWeather.ocean_timestep!, Const,
        Duplicated(progn, dprogn), Duplicated(diagn, ddiagn), Const(model))
    ...
    fd_vjp = FiniteDifferences.j′vp(
        central_fdm(5, 1), x -> ocean_timestep(progn_copy, x, model),
        dprogn_copy, diagn_copy)
    ...
    autodiff(Reverse, SpeedyWeather.land_timestep!, Const,
        Duplicated(progn, dprogn), Duplicated(diagn, ddiagn), Const(model))
    ...
    fd_vjp = FiniteDifferences.j′vp(
        central_fdm(5, 1), x -> land_timestep(progn_copy, x, model),
        dprogn_copy, diagn_copy)
    ...
    autodiff(Reverse, SpeedyWeather.dynamics_tendencies!, Const, Duplicated(diagn, ddiag),
        Duplicated(progn, dprogn), Const(lf2), Const(model))
    ...
    fd_vjp = FiniteDifferences.j′vp(
        central_fdm(5, 1), x -> dynamics_tendencies(diagn_copy, x, lf2, model),
        ddiag_copy, progn_copy)
    ...
    autodiff(Reverse, SpeedyWeather.implicit_correction!, Const,
        Duplicated(diagn, ddiag), Const(model.implicit), Duplicated(progn, dprogn))
    ...
    fd_vjp = FiniteDifferences.j′vp(
        central_fdm(9, 1), x -> implicit_correction(diagn_copy, model.implicit, x),
        ddiag_copy, progn_copy)
    ...
    autodiff(Reverse, SpeedyWeather.transform!, Const, Duplicated(diagn, ddiag),
        Duplicated(progn, dprogn), Const(lf2), Duplicated(model, make_zero(model)))
    ...
    fd_vjp = FiniteDifferences.j′vp(
        central_fdm(5, 1), x -> transform_diagn(diag_copy, x, lf2, model),
        ddiag_copy, progn_copy)

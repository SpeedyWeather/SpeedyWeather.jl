...
@testset "Differentiability: Complete Transform Enzyme" begin
    ...
    for (i_grid, grid_type) in enumerate(grid_types)
        spectral_grid = SpectralGrid(
            Grid = grid_type, trunc = 10, nlayers = 1, dealiasing = grid_dealiasing[i_grid])
        ...
        if fd_tests[i_grid]
            ...
            autodiff(Reverse, transform!, Const, Duplicated(specs, dspecs),
                Duplicated(field, dfield), Duplicated(S, dS))
            ...
            autodiff(Reverse, transform!, Const, Duplicated(field, dfield),
                Duplicated(specs, dspecs), Duplicated(S, dS))
        ...
        function transform_identity!(
                x_out::AbstractField, x::AbstractField, S::SpectralTransform{NF}) where {NF}
        ...
        autodiff(Reverse, transform_identity!, Const, Duplicated(field_out, dfield_out),
            Duplicated(field, dfield), Duplicated(S, dS))
        ...
        if fd_tests[i_grid]
            ...
            autodiff(Reverse, transform_identity!, Const,
                Duplicated(spec, dspec), Duplicated(S, dS))
...
@testset "Differentiability: Spectral Gradients Enzyme" begin
    for (i_grid, grid_type) in enumerate(grid_types)
        if fd_tests[i_grid]
            ...
            # curl test
            autodiff(Reverse, curl!, Const, Duplicated(cu, dcu),
            ...
            for i in 1:dv.spectrum.mmax
                @test all(Array(dv[:, 1])[i:(dv.spectrum.lmax - 1), i] .≈
                          complex(0, -(i-1)))
            end
            ...
            # finite difference comparision, seeded with a one adjoint to get the direct gradient
            fd_vjp = FiniteDifferences.j′vp(central_fdm(5, 1), x -> curl(x[1], x[2], S), dcu2, (
                u, v))
            ...
            autodiff(Reverse, divergence!, Const, Duplicated(div, ddiv),
                Duplicated(u, du), Duplicated(v, dv), Duplicated(S, dS))
            ...
            for i in 1:du.spectrum.mmax
                @test all(Array(du[:, 1])[i:(du.spectrum.lmax - 1), i] .≈
                          complex(i-1, -(i-1)))
            end
            ...
            fd_vjp = FiniteDifferences.j′vp(central_fdm(5, 1), x -> divergence(x[1], x[2], S), ddiv2, (
                u, v))
            ...
            autodiff(Reverse, SpeedyWeather.SpeedyTransforms.UV_from_vor!,
                Const, Duplicated(u, du), Duplicated(v, dv),
                Duplicated(vor, dvor), Duplicated(S, dS))
            ...
            autodiff(Reverse, SpeedyWeather.SpeedyTransforms.UV_from_vordiv!,
                Const, Duplicated(u, du), Duplicated(v, dv),
                Duplicated(vor, dvor), Duplicated(div, ddiv), Duplicated(S, dS))
            ...
            fd_vjp = FiniteDifferences.j′vp(
                central_fdm(5, 1), x -> uvfromvordiv(x[1], x[2], S), duv_input, (vor, div))
            ...
            autodiff(Reverse, SpeedyWeather.SpeedyTransforms.∇²!, Const,
                Duplicated(res_∇, dres_∇), Duplicated(vor, dvor), Duplicated(S, dS))
            ...
            dvor = zero(vor)
            autodiff(Reverse, SpeedyWeather.SpeedyTransforms.∇!, Const,
                Duplicated(zonal_gradient, dzonal_gradient),
                Duplicated(merid_gradient, dmerid_gradient),
                Duplicated(vor, dvor), Duplicated(S, dS))
            ...
            fd_vjp = FiniteDifferences.j′vp(
                central_fdm(5, 1), x -> ∇(x, S), (dmerid_gradient2, dzonal_gradient2), vor)

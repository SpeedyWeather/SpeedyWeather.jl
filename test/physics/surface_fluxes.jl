@testset "Prescribed surface sensible heat fluxes" begin
    for Model in (PrimitiveDryModel, PrimitiveWetModel)
        ...
        set!(simulation.prognostic_variables.ocean.sensible_heat_flux,
            (λ, ϕ) -> ϕ > 0 ? 10 : 0, model.geometry)
        ...
        simulation = initialize!(model)
        set!(simulation.prognostic_variables.land.sensible_heat_flux,
            (λ, ϕ) -> ϕ > 0 ? 100 : 0, model.geometry)
        run!(simulation, period = Day(1), output = true)
    end
end
...
@testset "Prescribed surface humidity fluxes" begin
    ...
    simulation = initialize!(model)
    set!(simulation.prognostic_variables.ocean.surface_humidity_flux,
        (λ, ϕ) -> ϕ > 0 ? 5e-5 : 0, model.geometry)
    ...
    simulation = initialize!(model);
    set!(simulation.prognostic_variables.land.surface_humidity_flux,
        (λ, ϕ) -> ϕ > 0 ? 5e-5 : 0, model.geometry)
    run!(simulation, period = Day(1))
end

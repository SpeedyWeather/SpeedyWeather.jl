...
grid_list = [
    FullGaussianGrid,
    # FullClenshawGrid,
    OctahedralGaussianGrid    # OctahedralClenshawGrid,    # HEALPixGrid,    # OctaHEALPixGrid,
]
...
function get_test_data(; trunc, nlayers, Grid, NF)
    ...
    spectral_grid_gpu = SpectralGrid(;
        NF, trunc, nlayers, Grid, architecture = SpeedyWeather.GPU, dealiasing = 3)
            ...
            @testset for Grid in grid_list
                @testset for NF in NFs
                    ...
                    SpeedyTransforms._legendre!(
                        g_north_cpu, g_south_cpu, spec_cpu, scratch, S_cpu)
                    # Copy to GPU

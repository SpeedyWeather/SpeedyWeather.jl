...
@testset "Output for BarotropicModel" begin
    ...
    for Grid in (FullGaussianGrid, FullClenshawGrid, OctahedralGaussianGrid,
        OctahedralClenshawGrid, HEALPixGrid, OctaHEALPixGrid)
...
@testset "Output for PrimitiveWetModel" begin
    ...
    add!(model,
        ...
        SpeedyWeather.SurfaceTemperatureOutput()        #         SpeedyWeather.MeanSeaLevelPressureOutput(),   # this should be default now        #         SpeedyWeather.SurfacePressureOutput(),        # don't output surface pressure too
    )
    ...
    @test maximum(abs.(ds["u10"].var[:, :, end])) <
          maximum(abs.(ds["u"].var[:, :, end, end]))
    @test maximum(abs.(ds["v10"].var[:, :, end])) <
          maximum(abs.(ds["u"].var[:, :, end, end]))
...
@testset "Restart from restart file without output" begin
    ...
    add!(model,
        :restart_file => RestartFile(
            path = tmp_output_path, write_only_with_output = false, filename = "myrestart.jld2"))
...
@testset "Time axis" begin
    ...
    spectral_grid = SpectralGrid()
    output = NetCDFOutput(spectral_grid, PrimitiveDry, path = tmp_output_path,
        id = "dense-output-test", output_dt = Hour(0))
    model = PrimitiveDryModel(spectral_grid; output)
    simulation = initialize!(model)
    run!(simulation, output = true; period = Day(1))
    ...
    @test t ==
          manual_time_axis(model.output.startdate, model.time_stepping.Δt_millisec, progn.clock.n_timesteps)

    # do a simulation with the adjust_Δt_with_output turned on 
    output = NetCDFOutput(spectral_grid, PrimitiveDry, path = tmp_output_path,
        id = "adjust_dt_with_output-test", output_dt = Minute(70))
    ...
    output = NetCDFOutput(spectral_grid, PrimitiveDry, path = tmp_output_path,
        id = "long-output-test", output_dt = Day(3650))
    model = PrimitiveDryModel(spectral_grid; output, time_stepping)
    simulation = initialize!(model)

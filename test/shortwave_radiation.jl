@testset "Shortwave radiation: solar!" begin
    @testset for NF in (Float32, Float64)
        ...
        @testset for i in 1:4
            ...
            @test_skip column.topsr ≈ topsr[i]
        end
    end
end

@testset "Shortwave radiation: sol_oz!" begin
    @testset for NF in (Float32, Float64)
        _, diagn, model = SpeedyWeather.initialize_speedy(NF, PrimitiveEquation, nlev = 4)
        nlev = diagn.nlev
        nband = model.parameters.nband
        ...
        @test_skip column.topsr ≈ 518.2271117159975
        @test_skip column.fsol ≈ 518.2271117159975
        @test_skip column.ozupp ≈ 6.99611021887365
        @test_skip column.ozone ≈ 15.666684
        @test_skip column.zenit ≈ 1.3500085311452612
        @test column.stratz ≈ 0.
    end
end

@testset "Shortwave radiation: cloud!" begin
    @testset for NF in (Float32, Float64)
        _, diagn, model = SpeedyWeather.initialize_speedy(NF, PrimitiveEquation, nlev = 4)
        nlev = diagn.nlev
        nband = model.parameters.nband
...
@testset "Shortwave radiation: radsw!" begin
    @testset for NF in (Float32, Float64)
        _, diagn, model = SpeedyWeather.initialize_speedy(NF, PrimitiveEquation, nlev = 4)
        nlev = diagn.nlev
        nband = model.parameters.nband
        ...
        @test_skip column.ssrd ≈ 385.7997293028816
        @test_skip column.ssr ≈ 192.8998646514408
        @test_skip column.tsr ≈ 315.10016
        @test_skip column.tau2 ≈ [0.9526258427031802 0.3788324531779398 1.0 1.0;
        ...
        @test_skip column.tend_t_rsw ≈ [11.947794979452055, 27.611640775148402, 42.565299927060124, 40.46336031165737]
    end
end
...
@testset "shortwave_radiation!" begin
    @testset for NF in (Float32, Float64)
        ...
        @test_skip isapprox(column.tend_t_rsw, [
            11.947794979452055, 27.611640775148402, 42.565299927060124, 40.46336031165737
            ], rtol=0.1)
        # Range is generally order 10^-5
        @test_skip column.temp_tend ≈ [7.189150408440864e-6, 1.2141337616807728e-5, 1.3196339560843619e-5, 1.5994200814647754e-5]
    end
end

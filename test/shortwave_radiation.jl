@testset "Parametrization: shortwave radiation" begin
    @testset "solar!" begin @testset for NF in (Float32, Float64)
        topsr = [0.0, 283.06, 352.56537, 378.8209880964894]
        lat = [-89.0, 0.0, 45.0, 89.0]
    ...
    @testset "sol_oz!" begin @testset for NF in (Float32, Float64)
        _, diagn, model = SpeedyWeather.initialize_speedy(NF, model = PrimitiveEquation,
                                                          nlev = 4)
        nlev = diagn.nlev
        nband = model.parameters.nband
        ...
        @test column.stratz ≈ 0.0
    end end
    @testset "cloud!" begin @testset for NF in (Float32, Float64)
        _, diagn, model = SpeedyWeather.initialize_speedy(NF, model = PrimitiveEquation,
                                                          nlev = 4)
        nlev = diagn.nlev
        nband = model.parameters.nband
        ...
        column = ColumnVariables{NF}(nlev = nlev, nband = nband)
        column.humid .= [0.0, 0.03124937, 0.9748285, 6.7846994] # g/kg
        column.rel_hum = [0.0, 1.0, 1.0, 0.8964701087948754]
        ...
        @test column.icltop ≈ 1.0
        @test column.cloudc ≈ 1.0
        @test column.clstr ≈ 0.13447051631923132
        @test column.qcloud ≈ 0.9748285
    end end
    @testset "radsw!" begin @testset for NF in (Float32, Float64)
        _, diagn, model = SpeedyWeather.initialize_speedy(NF, model = PrimitiveEquation,
                                                          nlev = 4)
        nlev = diagn.nlev
        nband = model.parameters.nband
    ...
    @testset "shortwave_radiation!" begin @testset for NF in (Float32, Float64)
        _, diagn, model = SpeedyWeather.initialize_speedy(NF, model = PrimitiveEquation,
                                                          nlev = 4)
        nlev = diagn.nlev
        nband = model.parameters.nband
        ...
        @test isapprox(column.tend_t_rsw,
                       [
                           11.947794979452055, 27.611640775148402, 42.565299927060124,
                           40.46336031165737,
                       ], rtol = 0.1)
        # Range is generally order 10^-5
        @test column.temp_tend ≈ [
            7.189150408440864e-6,
            1.2141337616807728e-5,
            1.3196339560843619e-5,
            1.5994200814647754e-5,
        ]
    end end
end

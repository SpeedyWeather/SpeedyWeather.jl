...
function ismatching(arch::Type{<:AbstractArchitecture}, array_T::Type{<:AbstractArray})
    any(arch_array -> array_T <: arch_array, compatible_array_types(arch))
end
function ismatching(arch::AbstractArchitecture, array_T::Type{<:AbstractArray})
    ismatching(typeof(arch), array_T)
end
function ismatching(arch::AbstractArchitecture, array::AbstractArray)
    ismatching(arch, typeof(array))
end
...
function KernelAbstractions.synchronize(arch::AbstractArchitecture)
    KernelAbstractions.synchronize(arch.device)
end
end

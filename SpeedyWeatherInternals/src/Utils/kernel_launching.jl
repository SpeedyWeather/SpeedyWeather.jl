...
function work_layout(::Type{SpectralInnerWorkOrder}, worksize::NTuple{N, Int}) where {N}
    return heuristic_workgroup(worksize[1]-2, worksize[2:end]...),
    (worksize[1]-2, worksize[2:end]...)
end
...
function work_layout(::Type{DiagonalWorkOrder}, worksize::NTuple{N, Int}) where {N}
    return heuristic_workgroup(worksize[2], worksize[3:end]...),
    (worksize[2], worksize[3:end]...)
end
...
@inline function _launch!(
        arch, work_order::Type{<:AbstractWorkOrder}, worksize, kernel!, kernel_args...)

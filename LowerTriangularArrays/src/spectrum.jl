...
"""
function Spectrum(trunc::Integer; one_degree_more = false, kwargs...)
...
"""
function Spectrum(spectrum::Spectrum; architecture::AbstractArchitecture = DEFAULT_ARCHITECTURE())
    adapt(array_type(architecture),
...
Base.:(==)(s1::Spectrum, s2::Spectrum) = s1.lmax == s2.lmax && s1.mmax == s2.mmax
...
function Architectures.ismatching(s::Spectrum, array_type::Type{<:AbstractArray})
    ismatching(s.architecture, array_type)
end
function Architectures.ismatching(s::Spectrum, array::AbstractArray)
    ismatching(s.architecture, typeof(array))
end

Adapt.@adapt_structure Spectrum

Architectures.architecture(s::Spectrum) = s.architecture
function Architectures.on_architecture(architecture::AbstractArchitecture, s::Spectrum)

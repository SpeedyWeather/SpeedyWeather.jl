...
triangular matrices."""
struct LowerTriangularArray{
    T, N, ArrayType <: AbstractArray{T, N}, S <: AbstractSpectrum} <: AbstractArray{T, N}
    ...
    function LowerTriangularArray{T, N, ArrayType, S}(data::AbstractArray,
            spectrum::S) where {
            T, N, ArrayType <: AbstractArray{T, N}, S <: AbstractSpectrum}
...
function check_lta_input_array(data, spectrum, N)
    (ndims(data) == N) & (length(data) == prod(size(data)[2:end]) * nonzeros(spectrum))
end # & ismatching(spectrum, typeof(data)) TODO: reactivate this? problem for constructors
...
function LowerTriangularArray(data::ArrayType,
        spectrum::S) where {T, N, ArrayType <: AbstractArray{T, N}, S <: AbstractSpectrum}
    LowerTriangularArray{T, N, ArrayType, S}(data, spectrum)
end

# or construct using LowerTriangularMatrix for 1D arrays
function LowerTriangularMatrix(data::ArrayType,
        spectrum::S) where {T, ArrayType <: AbstractArray{T, 1}, S <: AbstractSpectrum}
    LowerTriangularArray{T, 1, ArrayType, S}(data, spectrum)
end

function LowerTriangularArray(data::ArrayType, lmax::Integer,
        mmax::Integer) where {T, N, ArrayType <: AbstractArray{T, N}}
...
function LowerTriangularMatrix(data::ArrayType, lmax::Integer,
        mmax::Integer) where {T, ArrayType <: AbstractArray{T, 1}}
...
function matrix_size(data::AbstractArray, spectrum::AbstractSpectrum)
    matrix_size(data, spectrum.lmax, spectrum.mmax)
end
function matrix_size(data::AbstractArray, spectrum::AbstractSpectrum, i::Integer)
    matrix_size(data, spectrum.lmax, spectrum.mmax, i)
end
function matrix_size(data::AbstractArray, lmax::Integer, mmax::Integer)
    (lmax, mmax, size(data)[2:end]...)
end
...
function Base.size(L::LowerTriangularArray, base::Type{<:IndexBasis} = OneBased; as = Vector)
    size(L, base, as)
end
function Base.size(L::LowerTriangularArray, i::Integer, base::Type{<:IndexBasis} = OneBased; as = Vector)
    size(L, i, base, as)
end
...
function Base.size(L::LowerTriangularArray, base::Type{OneBased}, as::Type{Matrix})
    matrix_size(L.data, L.spectrum.lmax, L.spectrum.mmax)
end
function Base.size(L::LowerTriangularArray, base::Type{ZeroBased}, as::Type{Matrix})
    matrix_size(L.data, L.spectrum.lmax-1, L.spectrum.mmax-1)
end
...
function Base.size(L::LowerTriangularArray, i::Integer, base::Type{OneBased}, as::Type{Matrix})
    matrix_size(L.data, L.spectrum.lmax, L.spectrum.mmax, i)
end
function Base.size(L::LowerTriangularArray, i::Integer, base::Type{ZeroBased}, as::Type{Matrix})
    matrix_size(L.data, L.spectrum.lmax-1, L.spectrum.mmax-1, i)
end
function Base.size(L::LowerTriangularArray, i::Integer, base::Type{OneBased}, as::Type{Vector})
    size(L.data, i)
end
...
function Base.array_summary(
        io::IO, L::LowerTriangularMatrix{T, A}, inds::Tuple{Vararg{Base.OneTo}}) where {
        T, A}
    mn = size(L; as = Matrix)
...
function Base.array_summary(io::IO, ::LowerTriangularArray{T, N, A},
        inds::Tuple{Vararg{Base.OneTo}}) where {T, N, A}
    A_ = nonparametric_type(A)
...
for f in (:zeros, :ones, :rand, :randn)
    @eval begin
        ...
        function Base.$f(
            ...
            return LowerTriangularArray(ArrayType_($f(T, nonzeros(lmax, mmax), I...)),
                Spectrum(lmax, mmax, architecture = architecture(ArrayType_)))
        end
        ...
        function Base.$f(
            ...
            return LowerTriangularArray($f(T, nonzeros(lmax, mmax), I...),
                Spectrum(lmax, mmax, architecture = architecture(Array)))
        end
        ...
        function Base.$f(
            ...
            return LowerTriangularMatrix($f(T, nonzeros(lmax, mmax)),
                Spectrum(lmax, mmax, architecture = architecture(Array)))
        end
        ...
        function Base.$f(
            ...
            return LowerTriangularArray(
                array_type(spectrum.architecture)($f(T, nonzeros(spectrum), I...)),
                spectrum)
        end
        ...
        Base.$f(::Type{LowerTriangularArray}, lmax::Integer,
            mk::Integer...) = $f(LowerTriangularArray{Float64}, lmax, mk...)
        Base.$f(::Type{LowerTriangularMatrix}, lmax::Integer,
            mmax::Integer) = $f(LowerTriangularArray{Float64}, lmax, mmax)

        Base.$f(::Type{LowerTriangularArray},
            spectrum::AbstractSpectrum,
            I::Vararg{Integer, M}) where {M} = $f(LowerTriangularArray{Float64}, spectrum, I...)
        Base.$f(::Type{LowerTriangularMatrix},
            spectrum::AbstractSpectrum) = $f(LowerTriangularArray{Float64}, spectrum)
        Base.$f(spectrum::AbstractSpectrum,
            I::Vararg{Integer, M}) where {M} = $f(LowerTriangularArray{Float64}, spectrum, I...)
    end
end

function Base.zero(L::LTA) where {LTA <: LowerTriangularArray}
    zeros(LTA, L.spectrum, size(L)[2:end]...)
end
function Base.one(L::LTA) where {LTA <: LowerTriangularArray}
    ones(LTA, L.spectrum, size(L)[2:end]...)
end
...
function LowerTriangularArray{T, N, ArrayType, S}(
    ...
    return LowerTriangularArray(ArrayType(undef, nonzeros(I[1], I[2]), I[3:end]...),
        Spectrum(I[1], I[2], architecture = architecture(ArrayType)))
end
...
@inline Base.getindex(L::LowerTriangularArray{T, N}, I::Vararg{
    Any, N}) where {T, N} = getindex(L.data, I...)
...
@inline function Base.getindex(L::LowerTriangularArray{T, N}, I::Vararg{
        Any, M}) where {T, N, M}
    l, m = I[1:2]
    @boundscheck (0 < l <= L.spectrum.lmax && 0 < m <= L.spectrum.mmax) ||
                 throw(BoundsError(L, (l, m)))
...
Base.@propagate_inbounds Base.getindex(L::LowerTriangularArray{T, N},
    I::CartesianIndex{M}) where {T, N, M} = getindex(L, Tuple(I)...)
Base.@propagate_inbounds Base.getindex(
    L::LowerTriangularArray{T, N}, I::CartesianIndex{N}) where {T, N} = getindex(L.data, I)

Base.@propagate_inbounds Base.getindex(
    L::LowerTriangularArray{T, N}, i::Integer) where {T, N} = getindex(L.data, i)

# needed to remove ambigouities in 1D case 
Base.@propagate_inbounds Base.getindex(L::LowerTriangularArray{T, 1, V, S},
    i::Integer) where {
    S <: AbstractSpectrum, V <: AbstractVector{T}} where {T} = getindex(L.data, i)
Base.@propagate_inbounds Base.getindex(L::LowerTriangularArray{T, 1, V, S},
    I::CartesianIndex{M}) where {
    T, S <: AbstractSpectrum, V <: AbstractVector{T}, M} = getindex(L, Tuple(I)...)
Base.@propagate_inbounds Base.getindex(L::LowerTriangularArray{T, 1, V, S},
    i::Integer,
    I::CartesianIndex{0}) where {
    T, S <: AbstractSpectrum, V <: AbstractVector{T}} = getindex(L, i)
Base.@propagate_inbounds Base.getindex(L::LowerTriangularArray{T, 1, V, S},
    i::Integer,
    I::CartesianIndices{0}) where {
    T, S <: AbstractSpectrum, V <: AbstractVector{T}} = getindex(L, i)

# setindex with lm, ..
@inline Base.setindex!(L::LowerTriangularArray{T, N}, x,
    I::Vararg{Any, N}) where {T, N} = setindex!(L.data, x, I...)

# setindex with il, im, ..
@inline function Base.setindex!(L::LowerTriangularArray{T, N}, x, I::Vararg{
        Any, M}) where {T, N, M}
...
@inline Base.setindex!(L::LowerTriangularArray{T, N}, x, i::CartesianIndex{0},
    I::Vararg{Any, M}) where {T, N, M} = setindex!(L, x, I)

@inline Base.setindex!(L::LowerTriangularArray, x, I::CartesianIndex) = setindex!(L, x, Tuple(I)...)
@inline Base.setindex!(L::LowerTriangularArray{T, N}, x,
    I::CartesianIndex{N}) where {T, N} = setindex!(L.data, x, I)
@inline Base.setindex!(L::LowerTriangularArray{T, N}, x, i::Integer) where {
    T, N} = setindex!(L.data, x, i)

# this is to avoid ambigouities
@inline Base.setindex!(L::LowerTriangularArray{T, 1, V, S},
    x,
    i::Integer) where {
    T, S <: AbstractSpectrum, V <: AbstractVector{T}} = setindex!(L.data, x, i)
@inline Base.setindex!(L::LowerTriangularArray{T, 1, V, S},
    x,
    I::CartesianIndex{1}) where {
    T, S <: AbstractSpectrum, V <: AbstractVector{T}} = setindex!(L.data, x, I)
@inline Base.setindex!(L::LowerTriangularArray{T, 1, V, S},
    x,
    I::CartesianIndex{2}) where {
    T, S <: AbstractSpectrum, V <: AbstractVector{T}} = setindex!(L, x, Tuple(I)...)
@inline Base.setindex!(L::LowerTriangularArray{T, 1, V, S},
    x,
    i::Integer,
    I::CartesianIndex{0}) where {
    T, S <: AbstractSpectrum, V <: AbstractVector{T}} = setindex!(L, x, i)
...
function eachharmonic(L1::LowerTriangularArray, Ls::LowerTriangularArray...)
    lowertriangular_match(L1, Ls...; horizontal_only = true) ||
        throw(DimensionMismatch(L1, Ls...))
...
function lowertriangular_match(L1::LowerTriangularArray, Ls::LowerTriangularArray...; kwargs...)
    ...
    return lowertriangular_match(L1, Ls[1]; kwargs...) &&
           lowertriangular_match(L1, Ls[2:end]...; kwargs...)
end
...
function LowerTriangularArray(M::ArrayType) where {T, N, ArrayType <: AbstractArray{T, N}}
    ...
    LowerTriangularArray(M[lowertriangle_indices(lmax, mmax), [Colon() for i in 1:(N - 2)]...], lmax, mmax)
end
...
function Base.copyto!(
    ...
    size(L1) != size(L2) &&
        return copyto!(L1, L2, Base.OneTo(minimum(size.((L1, L2), 1; as = Matrix))),
...
function Base.copyto!(
...
) where {T, S, N, SP1 <: AbstractSpectrum, SP2 <: AbstractSpectrum,
        ArrayType1 <: Array{T, N}, ArrayType2 <: Array{S, N}}
    ...
    @inbounds for m in 1:mmax
        for l in m:lmax
            lm += 1
            L1[lm, [Colon() for i in 1:(N - 1)]...] = (l in ls) && (m in ms) ?
                                                      T.(L2[l, m, [Colon()
                                                                            for i in 1:(N - 1)]...]) :
                                                      L1[lm, [Colon()
                                                                     for i in 1:(N - 1)]...]
...
function Base.copyto!(
        ...
        ArrayType1 <: AbstractArray{T}, ArrayType2 <: AbstractArray{S}}
...
function _copyto_core!(
    ...
    L1.data[ind_L1, [Colon() for i in 1:(N - 1)]...] = T.(L2.data[ind_L2, [Colon()
                                                                                           for i in 1:(N - 1)]...])
...
) where {T1, T2, N, S <: AbstractSpectrum, ArrayTypeT1 <: AbstractArray{T1},
        ArrayTypeT2 <: AbstractArray{T2}}
...
function Base.similar(
        L::LowerTriangularArray{
            T, N, ArrayType, SP}, I::Integer...) where {T, N, ArrayType, SP}
...
function Base.similar(L::LowerTriangularArray{T, N, ArrayType, SP},
        size::S) where {T, N, ArrayType, SP, S <: Tuple}
...
function Base.similar(
        L::LowerTriangularArray{
            S, N, ArrayType, SP}, ::Type{T}) where {T, S, N, SP, ArrayType}
    ArrayType_ = nonparametric_type(ArrayType)
...
function Base.similar(
        L::LowerTriangularArray{
            T, N, ArrayType, SP}, ::Type{T}) where {T, N, ArrayType, SP}
...
function Architectures.array_type(::Type{<:LowerTriangularArray{
        T, N, ArrayType}}) where {T, N, ArrayType}
    ArrayType
end
Architectures.array_type(L::LowerTriangularArray) = array_type(typeof(L))
function Architectures.ismatching(arch::AbstractArchitecture, L::LowerTriangularArray)
    ismatching(arch, L.data)
end
...
function Base.:(==)(L1::LowerTriangularArray, L2::LowerTriangularArray)
    L1.spectrum == L2.spectrum && L1.data == L2.data
end
function Base.isapprox(L1::LowerTriangularArray, L2::LowerTriangularArray; kwargs...)
    isapprox(L1.data, L2.data; kwargs...)
end
Base.all(L::LowerTriangularArray) = all(L.data)
Base.any(L::LowerTriangularArray) = any(L.data)

function Base.repeat(L::LowerTriangularArray, counts...)
    LowerTriangularArray(repeat(L.data, counts...), L.spectrum)
end
...
function lta_view(L::LowerTriangularArray, c::Colon, i, args...)
    LowerTriangularArray(view(L.data, c, i, args...), L.spectrum)
end
function lta_view(L::LowerTriangularMatrix, c::Colon)
    LowerTriangularArray(view(L.data, c), L.spectrum)
end
...
function BroadcastStyle(::Type{LowerTriangularArray{
        T, N, ArrayType, S}}) where {T, N, ArrayType, S}
...
function LowerTriangularStyle{N, ArrayType, S}(::Val{M}) where {N, ArrayType, S, M}
    LowerTriangularStyle{N, ArrayType, S}()
end
function LowerTriangularGPUStyle{N, ArrayType, S}(::Val{M}) where {N, ArrayType, S, M}
...
function Architectures.on_architecture(arch, L::LowerTriangularArray)
    ...
    if ismatching(L.spectrum, typeof(adapted_data)) # if matching, use the same spectrum 
        ...
        return LowerTriangularArray(adapted_data,
            Spectrum(L.spectrum, architecture = architecture(typeof(adapted_data))))
    end
end

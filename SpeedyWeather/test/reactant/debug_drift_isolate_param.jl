...
SpeedyWeather.column_parameterizations!(sim_c.variables, sim_c.model)
@jit SpeedyWeather.column_parameterizations!(sim_r.variables, sim_r.model)
println("[9] Comparing...")

sim_c.variables.tendencies.grid.temperature
dT_c = Array(sim_c.variables.tendencies.grid.temperature)
dT_r = Array(sim_r.variables.tendencies.grid.temperature)


dT_max = maximum(abs.(dT_c .- dT_r))

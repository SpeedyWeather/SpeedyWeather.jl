...
@testset "Complete Differentiability" begin
    ...
    #autodiff(Reverse, SpeedyWeather.timestep!, Const, Duplicated(vars, dvars), Const(dt), Duplicated(model, make_zero(model)), Const(lf1), Const(lf2))
    
    autodiff(Reverse, SpeedyWeather.timestep!, Const, Duplicated(vars, dvars), Const(dt), Const(model), Const(lf1), Const(lf2))

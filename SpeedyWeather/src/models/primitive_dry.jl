...
function variables(::Type{<:PrimitiveDry}, nsteps)
    return (
        ...
        PrognosticVariable(:divergence, Spectral4D(nsteps), desc = "Divergence", units = "1/s", fuse=:prog),
        PrognosticVariable(:temperature, Spectral4D(nsteps), desc = "Temperature", units = "K", fuse=:prog),
        PrognosticVariable(:pressure, Spectral3D(nsteps), desc = "Logarithm of surface pressure", units = "log(Pa)", fuse=:prog),

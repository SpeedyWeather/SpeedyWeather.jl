...
## [Output levels](@id output_levels)

By default, 3D variables are written on the model's vertical levels, sigma or hybrid
sigma-pressure depending on the model's
[Vertical coordinates](@ref vertical_coordinates_page), on a dimension called `layer`.
Pass `levels = PressureLevels()` to write them on pressure levels instead

```@example netcdf
spectral_grid = SpectralGrid(nlayers = 8)
output = NetCDFOutput(spectral_grid, PrimitiveWet, levels = PressureLevels())
output.levels
```

or choose the levels yourself, in Pa
...
The vertical dimension in the file is then called `pressure` and holds the levels in hPa,
...
[Vertical interpolation onto pressure levels](@ref vertical_interpolation). The default
`PressureLevels()` writes the levels 50, 100, 200, 300, 500, 700, 850, 925 and 1000 hPa.

`PressureLevels` interpolates linearly in log(pressure) by default, pass
...
Pressure levels can lie below the lowest model level, which sits some tens of hPa above
...
default) instead, while still extrapolating between the lowest model level and the

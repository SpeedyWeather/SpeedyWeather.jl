...
all other [parameterizations](parameterizations.md) in the primitive equation model without humidity.
One could also just switch off the boundary layer scheme which would also automatically turn off
the [surface fluxes](surface_fluxes.md) (heat and momentum) as they aren't supposed to run with Held-Suarez forcing.
...
- creating an `orography::NoOrography` which will have no [orography](orography.md) and zero surface geopotential.
...
of the [convection](convection.md) scheme, causing updrafts and downdrafts in both humidity and temperature.

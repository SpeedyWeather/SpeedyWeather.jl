...
We are currently working on making SpeedyWeather.jl compatible with the MLIR/XLA optimizer Reactant.jl. If you want to try it out, use the `ReactantDevice` architecture and the set your backend within Reatant with `Reactant.set_default_backend`. 

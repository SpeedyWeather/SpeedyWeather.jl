...
- `BarotropicModel` solves the [2D barotropic vorticity equations](@ref barotropic_vorticity_model)
- `ShallowWaterModel` solves the [2D shallow water equations](@ref shallow_water_model)
- `PrimitiveDryModel` solves the [3D primitive equations](@ref primitive_equation_model) without humidity
- `PrimitiveWetModel` solves the [3D primitive equations](@ref primitive_equation_model) with humidity

but each of these models has further modularity such that the equations being
solved also depend on the [model components](@ref create_model_components) being defined. For example,
...
`spectral_grid` (a [`SpectralGrid`](@ref)) defining the resolution, then additional keyword arguments
...
of [Variables](@ref) happens in [`initialize!(model)`](@ref initialize).
...
(see [Surface humidity fluxes](@ref)) aren't needed.

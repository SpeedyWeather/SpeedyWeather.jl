...
# south, covering every ring in that hemisphere at once) instead of one per ring — this
# is what actually eliminates the fixed ~0.07-0.08ms per-`encode!` Objective-C dispatch
# overhead identified as the dominant cost in `metal_profiling.md`, rather than one
# `encode!` per ring. Uses the per-ring gather/scatter `MtlArray` buffers from
# `MetalFourierCommandBatchCache` (`get_cache`).
# =====================================================================================
...
# separately (rather than sharing one commit) — benchmarked to consistently win or tie
# a shared commit across trunc=100..300, see metal_fourier_transform_status.md.

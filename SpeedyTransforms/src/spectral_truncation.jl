...
function spectral_truncation!(alms::LowerTriangularArray, trunc::Integer)
...
function spectral_truncation!(alms::LowerTriangularArray)
    spectral_truncation!(alms, size(alms, 2, ZeroBased, as = Matrix))
end
...
function spectral_truncation(
    ...
    (ltrunc > lmax || mtrunc > mmax) &&
        return spectral_interpolation(NF, alms, ltrunc, mtrunc)
...
function spectral_truncation(alms::LowerTriangularArray, ltrunc::Integer, mtrunc::Integer)
    spectral_truncation(eltype(alms), alms, ltrunc, mtrunc)
end
function spectral_truncation(alms::LowerTriangularArray, trunc::Integer)
    spectral_truncation(alms, trunc, trunc)
end
...
function spectral_interpolation(
    ...
    (ltrunc <= lmax && mtrunc <= mmax) &&
        return spectral_truncation(NF, alms, ltrunc, mtrunc)
...
function spectral_interpolation(alms::LowerTriangularArray, trunc::Integer)

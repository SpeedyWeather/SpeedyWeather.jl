...
function _fourier_serial!(                  # GRID TO SPECTRAL
    ...
    @inbounds for (k, k_grid) in zip(1:nlayers, eachlayer(field))
        for j_north in 1:nlat_half              # symmetry: loop over northern latitudes only
            ...
            ilons = rings[j_south]                      # in-ring indices southern ring
            _apply_serial_fft!(
                f_south, field, S, j, k, nfreq, ilons; not_equator = not_equator)
        end
    end
end
...
function _fourier_serial!(                  # SPECTRAL TO GRID
    ...
    @inbounds for (k, k_grid) in zip(1:nlayers, eachlayer(field))
        for j_north in 1:nlat_half              # symmetry: loop over northern latitudes only
            ...
            ilons = rings[j_south]              # in-ring indices southern ring
            _apply_serial_fft!(
                field, g_south, S, j, k, nfreq, ilons; not_equator = not_equator)
        end
    end
end

function which_FFT_package(::Type{<:AbstractArray{NF}}) where {NF <: AbstractFloat}
    NF <: Union{Float32, Float64} ? FFTW : GenericFFT
end

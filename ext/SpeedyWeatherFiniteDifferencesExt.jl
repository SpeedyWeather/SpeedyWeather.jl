...
function FiniteDifferences.to_vec(x::Vector{Particle{NF}}) where {NF}
    if isempty(x)
        ...
        function Vector_from_vec(x_vec)
            ...
            x_Vec = [backs[n](x_vec[(sz[n] - length(x_vecs[n]) + 1):sz[n]])
...
function FiniteDifferences.to_vec(x::T) where {T <: Union{
        PrognosticVariables, PrognosticVariablesOcean, PrognosticVariablesLand,
        DiagnosticVariables, Tendencies, GridVariables,
        DynamicsVariables, PhysicsVariables, ParticleVariables}}
    excluded_fields_pre, included_fields,
    excluded_fields_post = determine_included_fields(T)
...
function replace_NaN(x_type::T,
        vec) where {T <: Union{
        PrognosticVariablesOcean, PrognosticVariablesLand, PhysicsVariables}}

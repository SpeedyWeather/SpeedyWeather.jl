...
import SpeedyWeather.LowerTriangularArrays: lm2k

# range of the running indices lm in a l-column (degrees of spherical harmonics)
# given the column index m (order of harmonics) 
get_lm_range(m, lmax) = lm2k(2*m - 1, m, lmax):lm2k(lmax+m, m, lmax)
get_2lm_range(m, lmax) = 2*lm2k(2*m - 1, m, lmax)-1:2*lm2k(lmax+m, m, lmax)

...
function augmented_primal(config::EnzymeRules.RevConfigWidth{1}, func::Const{typeof(_fourier!)}, ::Type{<:Const}, 
    f_north::Duplicated, f_south::Duplicated, grids::Duplicated{<:AbstractGridArray}, scratch::Duplicated, S::Union{Const, MixedDuplicated}) 

    func.val(f_north.val, f_south.val, grids.val, scratch.val, S.val) # forward pass
...
function reverse(config::EnzymeRules.RevConfigWidth{1}, func::Const{typeof(_fourier!)}, ::Type{<:Const}, tape,
    ...
    _fourier!(dgridval, f_north.dval ./ scale, f_south.dval ./ scale, scratch.dval, S.val) # inverse FFT (w/o normalization)
    grids.dval .+= dgridval 
...
function augmented_primal(config::EnzymeRules.RevConfigWidth{1}, func::Const{typeof(_fourier!)}, ::Type{<:Const}, 
    grids::Duplicated{<:AbstractGridArray}, f_north::Duplicated, f_south::Duplicated, scratch::Duplicated, S::Union{Const, MixedDuplicated}) 

    func.val(grids.val, f_north.val, f_south.val, scratch.val, S.val) # forward pass
...
function reverse(config::EnzymeRules.RevConfigWidth{1}, func::Const{typeof(_fourier!)}, ::Type{<:Const}, tape,
    ...
    _fourier!(dfnorthval, dfsouthval, grids.dval, scratch.dval, S.val) # inverse FFT (w/o normalization)

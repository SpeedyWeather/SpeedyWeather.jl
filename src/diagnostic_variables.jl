...
function Base.zeros(::Type{GridVariables}, G::Geometry{NF}) where {NF}
    ...
    return GridVariables(vor_grid, div_grid, temp_grid, temp_virt_grid, humid_grid,
                         geopot_grid,
                         U_grid, V_grid)
end
...
function DiagnosticVariables(G::Geometry{NF}, S::SpectralTransform{NF}) where {NF}
    zeros(DiagnosticVariables, G, S)
end

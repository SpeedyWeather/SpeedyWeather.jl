...
# general version
function Base.zeros(::Type{PrognosticVariablesLayer{NF}}, lmax::Integer,
...
function Base.zeros(::Type{PrognosticVariablesLayer{NF}}, model::ModelSetup, lmax::Integer,
                    mmax::Integer) where {NF}
    # use one more l for size compat with vector quantities
    vor = has(model, :vor) ? zeros(LowerTriangularMatrix{Complex{NF}}, lmax + 2, mmax + 1) :
          LowerTriangularMatrix{Complex{NF}}(undef, 0, 0)
    div = has(model, :div) ? zeros(LowerTriangularMatrix{Complex{NF}}, lmax + 2, mmax + 1) :
          LowerTriangularMatrix{Complex{NF}}(undef, 0, 0)
    temp = has(model, :temp) ?
           zeros(LowerTriangularMatrix{Complex{NF}}, lmax + 2, mmax + 1) :
           LowerTriangularMatrix{Complex{NF}}(undef, 0, 0)
    humid = has(model, :humid) ?
            zeros(LowerTriangularMatrix{Complex{NF}}, lmax + 2, mmax + 1) :
            LowerTriangularMatrix{Complex{NF}}(undef, 0, 0)
...
function Base.zeros(::Type{PrognosticVariablesLeapfrog{NF}}, lmax::Integer,
                    mmax::Integer) where {NF}
    # use one more l for size compat with vector quantities
    return PrognosticVariablesLeapfrog([zeros(PrognosticVariablesLayer{NF}, lmax, mmax)
                                        for _ in 1:N_LEAPFROG])
end

function Base.zeros(::Type{SurfaceLeapfrog{NF}}, lmax::Integer, mmax::Integer) where {NF}
    # use one more l for size compat with vector quantities
    return SurfaceLeapfrog([zeros(LowerTriangularMatrix{Complex{NF}}, lmax + 2, mmax + 1)
                            for _ in 1:N_LEAPFROG])
end
...
function Base.zeros(::Type{PrognosticVariablesLeapfrog{NF}},
    ...
    return PrognosticVariablesLeapfrog([zeros(PrognosticVariablesLayer{NF}, model, lmax,
                                              mmax) for _ in 1:N_LEAPFROG])
end
...
function Base.zeros(::Type{PrognosticVariables{NF}},
    ...
    pres = has(model, :pres) ? zeros(SurfaceLeapfrog{NF}, lmax, mmax) :
           zeros(SurfaceLeapfrog{NF}, -2, -1)  # 2 leapfrog time steps for pres       
    return PrognosticVariables{NF, typeof(model)}(layers, pres, lmax, mmax, N_LEAPFROG,
                                                  nlev)
end
...
function _set_var_core!(var_old::LowerTriangularMatrix{T},
                        var_new::LowerTriangularMatrix{R}) where {T, R}
...
See [`set_var!`](@ref)
"""
function set_vorticity!(progn::PrognosticVariables, varargs...; kwargs...)
    set_var!(progn, :vor, varargs...; kwargs...)
end
...
See [`set_var!`](@ref)
"""
function set_divergence!(progn::PrognosticVariables, varargs...; kwargs...)
    set_var!(progn, :div, varargs...; kwargs...)
end
...
See [`set_var!`](@ref)
"""
function set_temperature!(progn::PrognosticVariables, varargs...; kwargs...)
    set_var!(progn, :temp, varargs...; kwargs...)
end
...
See [`set_var!`](@ref)
"""
function set_humidity!(progn::PrognosticVariables, varargs...; kwargs...)
    set_var!(progn, :humid, varargs...; kwargs...)
end
...
"""
function set_pressure!(progn::PrognosticVariables, pressure::AbstractGrid, M::ModelSetup;
                       lf::Integer = 1)
    set_pressure!(progn, spectral(pressure, M.spectral_transform); lf = lf)
end
...
Sets the prognostic variable with the surface pressure in grid space at leapfrog index `lf`.
"""
function set_pressure!(progn::PrognosticVariables, pressure::AbstractGrid, lf::Integer = 1)
    set_pressure!(progn, spectral(pressure); lf = lf)
end
...
Sets the prognostic variable with the surface pressure in grid space at leapfrog index `lf`.
"""
function set_pressure!(progn::PrognosticVariables, pressure::AbstractMatrix,
                       Grid::Type{<:AbstractGrid}, lf::Integer = 1)
    set_pressure!(progn, spectral(pressure, Grid); lf = lf)
end

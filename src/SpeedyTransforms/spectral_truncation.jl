...
function spectral_truncation!(  alms::LowerTriangularMatrix{NF},    # spectral field to be truncated
    ...
    lmax, mmax = size(alms; as=Matrix) .- 1         # 0-based degree l, order m of the legendre polynomials
...
function spectral_truncation(   ::Type{NF},                     # number format NF (can be complex)
    ...
    lmax, mmax = size(alms; as=Matrix) .- 1     # 0-based degree l, order m of the spherical harmonics

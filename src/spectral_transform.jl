...
function SpectralTransform(::Type{NF},                     # Number format NF
                           ...
                           legendre_shortcut::Symbol = :linear) where {NF}
    ...
    brfft_plans = [FFT_package.plan_brfft(zeros(Complex{NF}, nlon ÷ 2 + 1), nlon)
                   for nlon in nlons]
    ...
    Λs = [zeros(LowerTriangularMatrix{NF}, b * (lmax + 2), b * (mmax + 1))
          for _ in 1:nlat_half]
    ...
    vordiv_to_uv_x = LowerTriangularMatrix([-m / (l * (l + 1))
                                            for l in 0:(lmax + 1), m in 0:mmax])
...
function SpectralTransform(alms::AbstractMatrix{Complex{NF}};  # spectral coefficients
                           ...
                           Grid::Type{<:AbstractGrid} = FullGaussianGrid) where {NF}                          # number format NF
...
function SpectralTransform(map::AbstractGrid{NF};          # gridded field
                           recompute_legendre::Bool = true) where {NF}                      # number format NF
    Grid = typeof(map)
...
function get_recursion_factors(::Type{NF}, # number format NF
                               lmax::Int,  # max degree l of spherical harmonics (0-based here)
                               mmax::Int) where {NF <: AbstractFloat}
...
function gridded!(map::AbstractGrid{NF},                      # gridded output
                  ...
                  unscale_coslat::Bool = false) where {NF <: AbstractFloat}                 # number format NF
    ...
    @inbounds for j_north in 1:nlat_half    # symmetry: loop over northern latitudes only
        ...
        recompute_legendre &&
            Legendre.unsafe_legendre!(Λw, Λ, lmax, mmax, Float64(cos_colat[j_north]))
        ...
        not_equator &&
            LinearAlgebra.mul!(view(map.data, ilons), brfft_plan, view(gs, 1:nfreq))  # perform FFT
...
function spectral!(alms::LowerTriangularMatrix{Complex{NF}},   # output: spectral coefficients
                   ...
                   S::SpectralTransform{NF}) where {NF <: AbstractFloat}
    ...
    @inbounds for j_north in 1:nlat_half    # symmetry: loop over northern latitudes only
        ...
        not_equator ?
        LinearAlgebra.mul!(view(fs, 1:nfreq), rfft_plan, view(map.data, ilons)) :
        fill!(fs, 0)
        ...
        for m in 1:m_trunc                              # Σ_{m=0}^{mmax}, but 1-based index
            ...
            alms[lm_end] = even_degrees ? muladd(a_even, Λj[lm_end], alms[lm_end]) :
                           alms[lm_end]
...
function gridded(alms::AbstractMatrix,       # spectral coefficients
                 S::SpectralTransform{NF}) where {NF}                  # number format NF

...
function radset!(K::ParameterizationConstants,
    ...
    for jb in 1:4
        for jtemp in 100:199
            fband[jtemp, jb] = fband[200, jb]
        end
        for jtemp in 321:400
...
"""
function radlw_down!(column::ColumnVariables{NF},
    ...
    for k in 1:(n_stratosphere_levels - 1)
        column.st4a[k, 2] = NF(0.75) * temp[k] + NF(0.25) * column.st4a[k, 1]
        column.st4a[k + 1, 2] = NF(0.50) * temp[k + 1] +
                                NF(0.25) * (column.st4a[k, 1] + column.st4a[k + 1, 1])
    end
    ...
    for k in (n_stratosphere_levels + 1):(nlev - 1)
        column.st4a[k, 2] = anish * max(column.st4a[k, 1] - column.st4a[k - 1, 1], NF(0.0))
    end
    column.st4a[nlev, 2] = anis * max(temp[nlev] - column.st4a[nlev - 1, 1], NF(0.0))

    # Blackbody emission in the stratosphere
    for k in 1:n_stratosphere_levels
        column.st4a[k, 1] = sbc * column.st4a[k, 2]^4
        column.st4a[k, 2] = NF(0.0)
    end
    ...
    for k in (n_stratosphere_levels + 1):nlev
        ...
        column.st4a[k, 2] = NF(4.0) * st3a * column.st4a[k, 2]
    end
    ...
    for jb in 1:(nband - 2)
        emis = 1.0 - tau2[k, jb]
        brad = fband[convert(Int, temp[k]), jb] *
               (column.st4a[k, 1] + emis * column.st4a[k, 2])
        column.flux[jb] = emis * brad
        column.dfabs[k] = column.dfabs[k] - column.flux[jb]
    end
    column.flux[(nband - 1):nband] .= NF(0.0)
...
"""
function compute_bbe!(column::ColumnVariables{NF},
                      model::PrimitiveEquationModel) where {NF <: AbstractFloat}
...
"""
function radlw_up!(column::ColumnVariables{NF},
                   model::PrimitiveEquationModel) where {NF <: AbstractFloat}
    ...
    for jb in 1:nband
        for k in nlev:-1:n_stratosphere_levels
            emis = 1.0 - tau2[k, jb]

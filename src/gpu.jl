...
"""
function Device_KernelAbstractions()
    CUDA.functional() ? KernelAbstractions.CUDADevice : KernelAbstractions.CPU
end
...
function DeviceSetup()
    DeviceSetup(Device(), Device_KernelAbstractions(Device()), workgroup_size(Device()))
end
function DeviceSetup(device::AbstractDevice)
    DeviceSetup(device, Device_KernelAbstractions(device), workgroup_size(device))
end
function DeviceSetup(device::AbstractDevice, n::Integer)
    DeviceSetup(device, Device_KernelAbstractions(device), n)
end

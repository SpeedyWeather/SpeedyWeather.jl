...
"""
function diagnose_convection!(column::ColumnVariables{NF},
    ...
    if pres > pres_thresh_cnv
        ...
        sat_moist_static_energy_pbl = max(moist_static_energy[nlev],
                                          sat_moist_static_energy[nlev])

        # 2. Minimum of moist static energy in the lowest two levels
        moist_static_energy_lower_trop = min(moist_static_energy[nlev],
                                             moist_static_energy[nlev - 1])
        ...
        for k in (nlev - 3):-1:3
            ...
            if moist_static_energy_lower_trop > sat_moist_static_energy_half[k]
                ...
                column.excess_humidity = max(humid[nlev] - humid_threshold_pbl,
                                             (moist_static_energy[nlev] -
                                              sat_moist_static_energy_half[k]) / alhc)
            end
        end
...
"""
function convection!(column::ColumnVariables{NF},
                     model::PrimitiveEquationModel) where {NF <: AbstractFloat}
    ...
    mass_flux = Δp / (gravity * 3600humid_relax_time_cnv) *
    ...
    # Net flux
    net_flux_dry_static_energy[nlev] = flux_down_dry_static_energy -
                                       flux_up_dry_static_energy
    ...
    for k in (nlev - 1):-1:(cloud_top + 1)
        ...
        net_flux_dry_static_energy[k] = flux_up_dry_static_energy -
                                        flux_down_dry_static_energy
        ...
        net_flux_dry_static_energy[k] += flux_down_dry_static_energy -
                                         flux_up_dry_static_energy
    ...
    # Net flux of dry static energy and moisture
    net_flux_dry_static_energy[cloud_top] = flux_up_dry_static_energy -
                                            flux_down_dry_static_energy +
                                            alhc * precip_convection

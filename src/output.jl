...
function initialize_netcdf_output(progn::PrognosticVariables,
    ...
    var_time = NcVar("time", dim_time, t = Int64,
                     atts = Dict("units" => time_string, "long_name" => "time"))
    ...
    dim_lon = NcDim(lon_name, nlon, values = lond,
                    atts = Dict("units" => lon_units, "long_name" => lon_longname))
    dim_lat = NcDim(lat_name, nlat, values = latd,
                    atts = Dict("units" => lat_units, "long_name" => lat_longname))
    dim_lev = NcDim("lev", nlev, values = collect(Int32, 1:nlev),
                    atts = Dict("units" => "1", "long_name" => "vertical model levels"))
    ...
    all_ncvars = (time = var_time,
                  u = NcVar("u", [dim_lon, dim_lat, dim_lev, dim_time], t = output_NF,
                            compress = compression_level,
                            atts = Dict("long_name" => "zonal wind", "units" => "m/s",
                                        "missing_value" => missing_value,
                                        "_FillValue" => missing_value)),
                  v = NcVar("v", [dim_lon, dim_lat, dim_lev, dim_time], t = output_NF,
                            compress = compression_level,
                            atts = Dict("long_name" => "meridional wind", "units" => "m/s",
                                        "missing_value" => missing_value,
                                        "_FillValue" => missing_value)),
                  vor = NcVar("vor", [dim_lon, dim_lat, dim_lev, dim_time], t = output_NF,
                              compress = compression_level,
                              atts = Dict("long_name" => "relative vorticity",
                                          "units" => "1/s",
                                          "missing_value" => missing_value,
                                          "_FillValue" => missing_value)),
                  pres = NcVar("pres", [dim_lon, dim_lat, dim_time], t = output_NF,
                               compress = compression_level,
                               atts = Dict("long_name" => "interface displacement",
                                           "units" => "m", "missing_value" => missing_value,
                                           "_FillValue" => missing_value)),
                  div = NcVar("div", [dim_lon, dim_lat, dim_lev, dim_time], t = output_NF,
                              compress = compression_level,
                              atts = Dict("long_name" => "divergence", "units" => "1/s",
                                          "missing_value" => missing_value,
                                          "_FillValue" => missing_value)),
                  temp = NcVar("temp", [dim_lon, dim_lat, dim_lev, dim_time], t = output_NF,
                               compress = compression_level,
                               atts = Dict("long_name" => "temperature", "units" => "K",
                                           "missing_value" => missing_value,
                                           "_FillValue" => missing_value)),
                  humid = NcVar("humid", [dim_lon, dim_lat, dim_lev, dim_time],
                                t = output_NF, compress = compression_level,
                                atts = Dict("long_name" => "specific humidity",
                                            "units" => "1",
                                            "missing_value" => missing_value,
                                            "_FillValue" => missing_value)),
                  orog = NcVar("orog", [dim_lon, dim_lat], t = output_NF,
                               compress = compression_level,
                               atts = Dict("long_name" => "orography", "units" => "m",
                                           "missing_value" => missing_value,
                                           "_FillValue" => missing_value)))
    ...
    vars_out = [all_ncvars[key]
                for key in keys(all_ncvars) if key in vcat(:time, output_vars)]
    netcdf_file = NetCDF.create(joinpath(run_path, output_filename), vars_out,
...
function write_netcdf_variables!(outputter::Output,
    ...
    for (k, (progn_layer, diagn_layer)) in enumerate(zip(progn.layers, diagn.layers))
        ...
        elseif output_grid == :matrix
            ...
            MGs = ((M, G) for (M, G) in zip((u, v, vor, div, temp, humid),
                                            (U_grid, V_grid, vor_grid, div_grid, temp_grid,
                                             humid_grid))
                   if length(M) > 0)
        ...
        elseif output_grid == :full
            ...
            :humid in output_vars &&
                gridded!(Grid(humid), progn_layer.leapfrog[lf].humid, S)
        end
        ...
        :u in output_vars &&
            NetCDF.putvar(netcdf_file, "u", u, start = [1, 1, k, i], count = [-1, -1, 1, 1])
        :v in output_vars &&
            NetCDF.putvar(netcdf_file, "v", v, start = [1, 1, k, i], count = [-1, -1, 1, 1])
        :vor in output_vars && NetCDF.putvar(netcdf_file, "vor", vor, start = [1, 1, k, i],
                      count = [-1, -1, 1, 1])
        :div in output_vars && NetCDF.putvar(netcdf_file, "div", div, start = [1, 1, k, i],
                      count = [-1, -1, 1, 1])
        :temp in output_vars &&
            NetCDF.putvar(netcdf_file, "temp", temp, start = [1, 1, k, i],
                          count = [-1, -1, 1, 1])
        :humid in output_vars &&
            NetCDF.putvar(netcdf_file, "humid", humid, start = [1, 1, k, i],
                          count = [-1, -1, 1, 1])
    end
    ...
    if :pres in output_vars
        ...
        NetCDF.putvar(outputter.netcdf_file, "pres", pres, start = [1, 1, i],
                      count = [-1, -1, 1])
    end

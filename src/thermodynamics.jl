...
"""
function get_thermodynamics!(column::ColumnVariables{NF},
                             model::PrimitiveEquationModel{NF}) where {NF <: AbstractFloat}
...
"""
function interpolate!(column::ColumnVariables{NF},
    ...
    for (full, half) in ((humid, humid_half),
...
"""
function interpolate!(A_full_level::Vector{NF},
                      A_half_level::Vector{NF},
                      column::ColumnVariables{NF},
                      model::PrimitiveEquationModel) where {NF <: AbstractFloat}
    @unpack nlev = column
    ...
    A_half_level[nlev] = A_full_level[nlev] +
...
respectively.
"""
function saturation_vapour_pressure!(column::ColumnVariables{NF},
                                     model::PrimitiveEquationModel{NF}) where {
                                                                               NF <:
                                                                               AbstractFloat
                                                                               }
...
"""
function saturation_specific_humidity!(column::ColumnVariables{NF},
                                       model::PrimitiveEquationModel{NF}) where {
                                                                                 NF <:
                                                                                 AbstractFloat
                                                                                 }
    ...
    for k in eachlayer(column)
        ...
        sat_humid[k] = mol_ratio * sat_vap_pres[k] /
                       (pₖ - one_minus_mol_ratio * sat_vap_pres[k])
    end
...
"""
function moist_static_energy!(column::ColumnVariables{NF},
                              model::PrimitiveEquationModel{NF}) where {NF <: AbstractFloat}
...
"""
function saturation_moist_static_energy!(column::ColumnVariables{NF},
                                         model::PrimitiveEquationModel{NF}) where {
                                                                                   NF <:
                                                                                   AbstractFloat
                                                                                   }

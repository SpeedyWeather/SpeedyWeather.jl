...
function gaussian_weights(nlat_half::Integer)
    FastGaussQuadrature.gausslegendre(2nlat_half)[2][1:nlat_half]
end
function clenshaw_curtis_weights(nlat_half::Integer)
    nlat = 2nlat_half - 1
    θs = get_colat(FullClenshawGrid, nlat_half)
    return [4sin(θj) / (nlat + 1) * sum([sin(p * θj) / p for p in 1:2:nlat])
            for θj in θs[1:nlat_half]]
end

# QUADRATURE WEIGHTS (INEXACT), for HEALPix full grids
function healpix_weights(nlat_half::Integer)
    [nlon_healpix(nlat_half, j) for j in 1:nlat_half] .* (2 / npoints_healpix(nlat_half))
end
function healpix4_weights(nlat_half::Integer)
...
function get_solid_angles(Grid::Type{<:Union{HEALPixGrid, HEALPix4Grid}},
                          nlat_half::Integer)

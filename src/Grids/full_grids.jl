...
function Base.Matrix(grid::AbstractFullGrid{T}) where {T}
    Matrix{T}(reshape(grid.data, :, get_nlat(grid)))
end
matrix_size(grid::AbstractFullGrid) = (get_nlon_max(grid), get_nlat(grid))
function matrix_size(Grid::Type{<:AbstractFullGrid}, n::Integer)
    (get_nlon_max(Grid, n), get_nlat(Grid, n))
end
...
struct FullClenshawGrid{T} <: AbstractFullGrid{T}
    ...
    function FullClenshawGrid{T}(data, nlat_half) where {T}
        length(data) == npoints_clenshaw(nlat_half) ?
        new(data, nlat_half) :
...
function FullClenshawGrid{T}(data::AbstractVector) where {T}
    FullClenshawGrid{T}(data, nlat_half_clenshaw(length(data)))
end
function FullClenshawGrid(data::AbstractVector, n::Integer...)
    FullClenshawGrid{eltype(data)}(data, n...)
end

truncation_order(::Type{<:FullClenshawGrid}) = 3            # cubic
function get_truncation(::Type{<:FullClenshawGrid}, nlat_half::Integer)
    floor(Int, (4nlat_half - 1) / 4)
end
function get_resolution(::Type{<:FullClenshawGrid}, trunc::Integer)
    roundup_fft(ceil(Int, (4 * trunc + 1) / 4))
end
nlat_odd(::Type{<:FullClenshawGrid}) = true
get_npoints(::Type{<:FullClenshawGrid}, nlat_half::Integer) = npoints_clenshaw(nlat_half)
function get_colat(::Type{<:FullClenshawGrid}, nlat_half::Integer)
    [j / (2nlat_half) * π for j in 1:(2nlat_half - 1)]
end
...
struct FullGaussianGrid{T} <: AbstractFullGrid{T}
    ...
    function FullGaussianGrid{T}(data, nlat_half) where {T}
        length(data) == 8nlat_half^2 ?
        new(data, nlat_half) :
        error("$(length(data))-element Vector{$(eltype(data))} cannot be used to create a " *
...
# infer nlat_half from data vector length, infer parametric type from eltype of data
function FullGaussianGrid{T}(data::AbstractVector) where {T}
    FullGaussianGrid{T}(data, nlat_half_gaussian(length(data)))
end
function FullGaussianGrid(data::AbstractVector, n::Integer...)
    FullGaussianGrid{eltype(data)}(data, n...)
end

truncation_order(::Type{<:FullGaussianGrid}) = 2            # quadratic
function get_truncation(::Type{<:FullGaussianGrid}, nlat_half::Integer)
    floor(Int, (4nlat_half - 1) / 3)
end
function get_resolution(::Type{<:FullGaussianGrid}, trunc::Integer)
    roundup_fft(ceil(Int, (3 * trunc + 1) / 4))
end
nlat_odd(::Type{<:FullGaussianGrid}) = false
get_npoints(::Type{<:FullGaussianGrid}, nlat_half::Integer) = npoints_gaussian(nlat_half)
function get_colat(::Type{<:FullGaussianGrid}, nlat_half::Integer)
...
struct FullHEALPixGrid{T} <: AbstractFullGrid{T}
    ...
    function FullHEALPixGrid{T}(data, nlat_half) where {T}
        length(data) == npoints_fullhealpix(nlat_half) ?
        new(data, nlat_half) :
        error("$(length(data))-element Vector{$(eltype(data))} cannot be used to create a " *
...
# infer nlat_half from data vector length, infer parametric type from eltype of data
function FullHEALPixGrid{T}(data::AbstractVector) where {T}
    FullHEALPixGrid{T}(data, nlat_half_fullhealpix(length(data)))
end
function FullHEALPixGrid(data::AbstractVector, n::Integer...)
    FullHEALPixGrid{eltype(data)}(data, n...)
end
truncation_order(::Type{<:FullHEALPixGrid}) = 2            # quadratic
function get_truncation(::Type{<:FullHEALPixGrid}, nlat_half::Integer)
    get_truncation(HEALPixGrid, nlat_half)
end
function get_resolution(::Type{<:FullHEALPixGrid}, trunc::Integer)
    get_resolution(HEALPixGrid, trunc)
end
...
function get_quadrature_weights(::Type{<:FullHEALPixGrid}, nlat_half::Integer)
    healpix_weights(nlat_half)
end
...
struct FullHEALPix4Grid{T} <: AbstractFullGrid{T}
    ...
    function FullHEALPix4Grid{T}(data, nlat_half) where {T}
        length(data) == npoints_fullhealpix4(nlat_half) ?
        new(data, nlat_half) :
        error("$(length(data))-element Vector{$(eltype(data))} cannot be used to create a " *
...
# infer nlat_half from data vector length, infer parametric type from eltype of data
function FullHEALPix4Grid{T}(data::AbstractVector) where {T}
    FullHEALPix4Grid{T}(data, nlat_half_fullhealpix4(length(data)))
end
function FullHEALPix4Grid(data::AbstractVector, n::Integer...)
    FullHEALPix4Grid{eltype(data)}(data, n...)
end

truncation_order(::Type{<:FullHEALPix4Grid}) = 3            # cubic
function get_truncation(::Type{<:FullHEALPix4Grid}, nlat_half::Integer)
    floor(Int, (4nlat_half - 1) / 4)
end
function get_resolution(::Type{<:FullHEALPix4Grid}, trunc::Integer)
    roundup_fft(ceil(Int, (4 * trunc + 1) / 4))
end
nlat_odd(::Type{<:FullHEALPix4Grid}) = true
function get_npoints(::Type{<:FullHEALPix4Grid}, nlat_half::Integer)
    npoints_fullhealpix4(nlat_half)
end
function get_colat(::Type{<:FullHEALPix4Grid}, nlat_half::Integer)
    get_colat(HEALPix4Grid, nlat_half)
end
function get_quadrature_weights(::Type{<:FullHEALPix4Grid}, nlat_half::Integer)
    healpix4_weights(nlat_half)
end

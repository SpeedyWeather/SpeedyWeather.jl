...
function get_nlon_max(::Type{<:AbstractOctahedralGrid}, nlat_half::Integer)
    nlon_octahedral(nlat_half)
end

function get_nlon_per_ring(Grid::Type{<:AbstractOctahedralGrid}, nlat_half::Integer,
                           j::Integer)
...
function get_lon(G::Type{<:AbstractOctahedralGrid}, nlat_half::Integer)
    get_lon(full_grid(G), nlat_half)
end
...
struct OctahedralGaussianGrid{T} <: AbstractOctahedralGrid{T}
    ...
    function OctahedralGaussianGrid{T}(data, nlat_half) where {T}
        length(data) == npoints_octahedral(nlat_half, false) ?
        new(data, nlat_half) :
...
function npoints_octahedral(nlat_half::Integer, nlat_oddp::Bool)
    nlat_oddp ? max(0, 4nlat_half^2 + 32nlat_half - 16) : 4nlat_half^2 + 36nlat_half
end # max(0,...) needed to avoid negative array size when nlat_half==0
function nlat_half_octahedral(npoints::Integer, nlat_oddp::Bool)
    nlat_oddp ? round(Int, -4 + sqrt(20 + npoints / 4)) :
    round(Int, -9 / 2 + sqrt((9 / 2)^2 + npoints / 4))
end  # inverse
nlon_octahedral(j::Integer) = 16 + 4j

# infer nside from data vector length, infer parametric type from eltype of data
function OctahedralGaussianGrid{T}(data::AbstractVector) where {T}
    OctahedralGaussianGrid{T}(data, nlat_half_octahedral(length(data), false))
end
function OctahedralGaussianGrid(data::AbstractVector, n::Integer...)
    OctahedralGaussianGrid{eltype(data)}(data, n...)
end
...
function get_npoints(::Type{<:OctahedralGaussianGrid}, nlat_half::Integer)
    npoints_octahedral(nlat_half, false)
end
function get_colat(::Type{<:OctahedralGaussianGrid}, nlat_half::Integer)
    get_colat(FullGaussianGrid, nlat_half)
end
...
struct OctahedralClenshawGrid{T} <: AbstractOctahedralGrid{T}
    ...
    # check that `nlat_half` match the vector `v` length
    function OctahedralClenshawGrid{T}(data, nlat_half) where {T}
        length(data) == npoints_octahedral(nlat_half, true) ?
        new(data, nlat_half) :
        error("$(length(data))-element Vector{$(eltype(data))}" *
...
function OctahedralClenshawGrid{T}(data::AbstractVector) where {T}
    OctahedralClenshawGrid{T}(data,
                              nlat_half_octahedral(length(data), true))
end
function OctahedralClenshawGrid(data::AbstractVector, n::Integer...)
    OctahedralClenshawGrid{eltype(data)}(data, n...)
end
...
function get_npoints(::Type{<:OctahedralClenshawGrid}, nlat_half::Integer)
    npoints_octahedral(nlat_half, true)
end
function get_colat(::Type{<:OctahedralClenshawGrid}, nlat_half::Integer)
    get_colat(FullClenshawGrid, nlat_half)
end
function get_quadrature_weights(::Type{<:OctahedralClenshawGrid}, nlat_half::Integer)
...
function matrix_size(::Type{OctahedralClenshawGrid}, nlat_half::Integer)
    (2 * (4 + nlat_half), 2 * (4 + nlat_half))
end
function Base.Matrix(G::OctahedralClenshawGrid{T}; kwargs...) where {T}
...
function Matrix!(M::AbstractMatrix, G::OctahedralClenshawGrid; kwargs...)
    Matrix!((M, G); kwargs...)
end

...
function surface_heat_flux!(
    ...
    # drag coefficient
    drag_ocean = heat_flux.use_boundary_layer_drag ? column.boundary_layer_drag :
                 heat_exchange

    # SPEEDY documentation Eq. 54/56, land/sea fraction included
    # Only flux from sea if available (not NaN) otherwise zero flux
    flux_ocean = isfinite(T_skin_ocean) ? ρ*drag_ocean*V₀*cₚ*(T_skin_ocean - T) :
                 zero(T_skin_ocean)
...
function surface_heat_flux!(
    ...
    # drag coefficient
    drag_land = heat_flux.use_boundary_layer_drag ? column.boundary_layer_drag :
                heat_exchange

    # SPEEDY documentation Eq. 54/56, land/sea fraction included
    # Only flux from sea if available (not NaN) otherwise zero flux
    flux_land = isfinite(T_skin_land) ? ρ*drag_land*V₀*cₚ*(T_skin_land - T) :
                zero(T_skin_land)

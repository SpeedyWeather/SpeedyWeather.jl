...
surface_heat_flux!(
    c::ColumnVariables, f::AbstractSurfaceHeatFlux,
    p::PrognosticVariables, m::PrimitiveEquation
) = surface_heat_flux!(c, f, m)

surface_humidity_flux!(
    c::ColumnVariables, f::AbstractSurfaceHumidityFlux,
    p::PrognosticVariables, m::PrimitiveEquation
...
function surface_fluxes!(
    ...
    return model isa PrimitiveWet && surface_humidity_flux!(column, model.surface_humidity_flux, progn, model)
end
...
function surface_thermodynamics!(
        column::ColumnVariables,
    ...
    return column.surface_air_density = column.pres[end] / (R_dry * Tᵥ)
end
...
function surface_thermodynamics!(
    ...
    column.surface_temp = column.temp[end]   # todo use constant POTENTIAL temperature
    return column.surface_air_density = column.pres[end] / (R_dry * column.surface_temp)
end

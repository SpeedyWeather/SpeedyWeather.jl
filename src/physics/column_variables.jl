...
function get_column!(
    ...
    return C.albedo_land = D.physics.land.albedo[ij]
end
...
function get_column!(
    ...
    return get_column!(C, D, P, ij, jring, model)
end

function get_column(
        S::AbstractSimulation,
        ij::Integer,
        verbose::Bool = true
    )
...
function write_column_tendencies!(
    ...
    return write_column_tendencies!(diagn, column, model.planet, model.atmosphere, ij)
end
...
function write_column_tendencies!(
    ...
    diagn.physics.total_precipitation_rate[ij] =
        ...
        column.rain_rate_large_scale + column.rain_rate_convection +
            column.snow_rate_large_scale + column.snow_rate_convection
    ) * ρ

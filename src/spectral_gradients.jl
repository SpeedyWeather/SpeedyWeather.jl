...
radius R. Returns `coslat_u`, i.e. the gradient ∂Ψ/∂θ with an additional cosine of latitude θ scaling.
...
function gradient_latitude!(coslat_u::AbstractArray{Complex{NF}},   # output: cos(lat)*zonal velocity u
                            Ψ::AbstractArray{Complex{NF}},          # input: streamfunction Ψ
                            ϵlms::AbstractArray{NF},                # recursion factors
                            R::Real=1,                              # radius of the sphere/Earth
                            ) where {NF<:AbstractFloat}             # number format NF
...
function gradient_latitude!(coslat_u::AbstractArray{Complex{NF}},   # output: cos(lat)*u
    ...
    lmax,mmax = size(Ψ) .- 1                                        # degree l, order m of spherical harmonics   
    ϵlms = get_recursion_factors(lmax,mmax)                         # precalculate recursion factors
...
function gradient_latitude( Ψ::AbstractArray{Complex{NF}},  # input: streamfunction Ψ
    ...
    lmax,mmax = size(Ψ) .- 1                                # degree l, order m of spherical harmonics
    coslat_u = zeros(Complex{NF},lmax+2,mmax+1)               # preallocate output, one more l for recursion
    return gradient_latitude!(coslat_u,Ψ,R)                   # call in-place version
end

"""
    coslat_∂alms_∂ϕ = gradient_longitude!(  coslat_∂alms_∂ϕ::AbstractMatrix{Complex{NF}},
...
While the zonal gradient is 1/coslat*∂/∂ϕ in spherical coordinates, this functions omits the 1/coslat scaling
such that the return array is coslat*∂alms/∂ϕ.
"""
function gradient_longitude!(   coslat_∂alms_∂ϕ::AbstractMatrix{Complex{NF}}, # output: coslat*zonal gradient
                                alms::AbstractMatrix{Complex{NF}};          # input: spectral coefficients
                                R::Real=1                                   # radius of the sphere/Earth
                                ) where {NF<:AbstractFloat}                 # number format NF

    @boundscheck size(alms) == size(coslat_∂alms_∂ϕ) || throw(BoundsError)
    ...
    @inbounds for m in 1:mmax+1                         # loop over all coefficients, order m
        for l in m:lmax+1                               # degree l
            coslat_∂alms_∂ϕ[l,m] = (m-1)*iR⁻¹*alms[l,m]   # gradient in lon = *i*m/R but order m is 1-based
        end
    end

    return coslat_∂alms_∂ϕ
end
...
Step 5 (spectral space): Compute gradients ∂/∂ϕ(uω/coslat) and ∂/∂θ(vω/coslat)
Step 6 (spectral space): Add ∂/∂ϕ(uω/coslat)+∂/∂θ(vω/coslat) and return.
"""
function divergence_uvω_spectral(   u_grid::AbstractMatrix{NF},     # zonal velocity in grid space
    ...
    ∂vω_∂θ = gradient_latitude(vω_coslat⁻¹)                     # TODO always correct coslat scaling?

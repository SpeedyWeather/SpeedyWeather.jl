...
function set!(
    ...
    isnothing(temp) ||
        set!(get_step(progn.temp, lf), temp, geometry, spectral_transform; add)
    isnothing(humid) ||
        set!(get_step(progn.humid, lf), humid, geometry, spectral_transform; add)
    isnothing(pres) ||
        set!(get_step(progn.pres, lf), pres, geometry, spectral_transform; add)

    # or provide u, v instead of vor, div
    isnothing(u) | isnothing(v) ||
        set_vordiv!(get_step(progn.vor, lf), get_step(progn.div, lf), u, v,
            geometry, spectral_transform; add, coslat_scaling_included)

    # OCEAN
    isnothing(sea_surface_temperature) || set!(progn.ocean.sea_surface_temperature,
        sea_surface_temperature, geometry, spectral_transform; add)
    isnothing(sea_ice_concentration) ||
        set!(progn.ocean.sea_ice_concentration, sea_ice_concentration,
            geometry, spectral_transform; add)

    # LAND
    isnothing(soil_temperature) || set!(progn.land.soil_temperature, soil_temperature,
        geometry, spectral_transform; add)
    isnothing(snow_depth) ||
        set!(progn.land.snow_depth, snow_depth, geometry, spectral_transform; add)
    isnothing(soil_moisture) ||
        set!(progn.land.soil_moisture, soil_moisture, geometry, spectral_transform; add)
...
@kernel inbounds=true function set_field_3d_kernel!(var, @Const(londs), @Const(latds),
        @Const(σ_levels_full), @Const(f), @Const(kernel_func))
    # Get indices
...
function set_vordiv!(
    ...
    u_ = coslat_scaling_included ? u :
         transform(RingGrids.scale_coslat⁻¹(transform(u, S)), S)
    v_ = coslat_scaling_included ? v :
         transform(RingGrids.scale_coslat⁻¹(transform(u, S)), S)
...
function set!(S::AbstractSimulation; kwargs...)
    set!(S.prognostic_variables, S.model.geometry;
        spectral_transform = S.model.spectral_transform, kwargs...)
end

function set!(progn::PrognosticVariables, model::AbstractModel; kwargs...)
    progn.scale[] != 1 &&
        @warn "Prognostic variables are scaled with $(progn.scale[]), but `set!` assumes unscaled variables."

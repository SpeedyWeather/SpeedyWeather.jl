...
function initialize!(particles::AbstractVector{P}, progn, diagn, model) where {P <:
                                                                               Particle}
...
function particle_advection!(progn, diagn, model)
    particle_advection!(progn, diagn, model.particle_advection, model)
end
...
function ParticleAdvection2D(SG::SpectralGrid; kwargs...)
    SG.nparticles == 0 &&
        @warn "ParticleAdvection2D created but nparticles = 0 in spectral grid."
...
function initialize!(
    ...
    nlayers < layer &&
        @warn "Particle advection on layer $layer on spectral grid with nlayers=$nlayers."

...
function initialize!(
    ...
    fill!(σ_lnp_A[1:1], 0)  # the corresponding sum is 1:k-1 so 0 to replace log(0) from above
    ...
    fill!(σ_lnp_B[1:1], σ_levels_half[1] <= 0 ? log(2) : σ_lnp_B[1])    # set α₁ = log(2), eq. 3.19

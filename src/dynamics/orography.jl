...
function initialize!(orog::ZonalRidge,
    ...
    for ij in eachindex(φ, orography)
        ...
        orography[ij] = g⁻¹*A*(A*(-2*sinφ^6*(cosφ^2 + 1/3) + 10/63) +
                               (8/5*cosφ^3*(sinφ^2 + 2/3) - π/4)*RΩ)
    end
...
function initialize!(orog::EarthOrography,
    ...
    orography_highres = on_architecture(
        S.architecture, FullGaussianField(ncfile["orog"].var[:, :], input_as = Matrix))

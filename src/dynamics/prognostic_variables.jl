...
@kwdef struct PrognosticVariables{
    ...
    ocean::PrognosticVariablesOcean{NF,
        ArrayType,
        GridType,
        GridVariable2D} = PrognosticVariablesOcean{NF, ArrayType, GridType, GridVariable2D}(;
        grid)

    "Land variables, soil temperature, snow, and soil moisture"
    land::PrognosticVariablesLand{NF,
        ArrayType,
        GridType,
        GridVariable2D,
        GridVariable3D} = PrognosticVariablesLand{
        NF, ArrayType, GridType, GridVariable2D, GridVariable3D}(;
        grid, nlayers = nlayers_soil)
...
"""$(TYPEDSIGNATURES)
Generator function."""
function PrognosticVariables(SG::SpectralGrid{Architecture, SpectrumType, GridType};
        nsteps = DEFAULT_NSTEPS) where {Architecture, SpectrumType, GridType}
    (; spectrum, grid, nlayers, nlayers_soil, nparticles) = SG
    (; NF, ArrayType) = SG
    (; SpectralVariable2D, SpectralVariable3D, SpectralVariable4D,
        GridVariable2D, GridVariable3D, ParticleVector) = SG

    return PrognosticVariables{NF, ArrayType, SpectrumType, GridType,
        SpectralVariable2D, SpectralVariable3D, SpectralVariable4D,
        GridVariable2D, GridVariable3D, ParticleVector}(;
...
function add!(
        progn::PrognosticVariables{
            NF, ArrayType, SpectrumType, GridType, SpectralVariable2D,
            SpectralVariable3D, SpectralVariable4D},
        tracers::Tracer...
) where {
        NF,                     # number format
        ArrayType,

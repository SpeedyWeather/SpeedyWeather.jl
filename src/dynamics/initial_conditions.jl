...
function initialize!(progn::PrognosticVariables{NF},
    ...
    sinφc,
    cosφc = sind(initial_conditions.perturb_lat), cosd(initial_conditions.perturb_lat)
    (; radius) = model.planet
    ...
    function vor(λ, φ, η)               # longitude, latitude (degree), sigma coordinate
        ...
        perturbation = perturb_uₚ/radius * exp(-(r/R)^2) *
                       (tand(φ) -
                        2*(radius/R)^2*acos(X)*(sinφc*cosd(φ) -
                                                cosφc*sind(φ)*cosd(λ-λc))/sqrt(1-X^2))
    ...
    function div(λ, φ, η)               # longitude, latitude (degree), sigma coordinate
        ...
        # Eq (13)
        return -2perturb_uₚ*radius/R^2 * exp(-(r/R)^2) * acos(X)/sqrt(1-X^2) * cosφc *
               sind(λ-λc)
    end
...
function initialize!(
    ...
    A(λ,
        θ) = ω/2 * (2Ω+ω) * cosd(θ)^2 +
             K^2/4*cosd(θ)^(2m)*((m+1)*cosd(θ)^2 + (2m^2-m-2) - 2m^2/(cosd(θ)^2))
...
@kernel inbounds=true function jablonowski_temperature_kernel!(
    ...
    temp_grid[ij, k] = Tη[k] +
                       A1 * ((-2sinφ^6 * (cosφ^2 + 1/3) + 10/63) * A2 +
...
function initialize!(progn::PrognosticVariables,
    ...
    (; atmosphere) = model
    lapse_rate = model isa PrimitiveDry ? atmosphere.dry_lapse_rate :

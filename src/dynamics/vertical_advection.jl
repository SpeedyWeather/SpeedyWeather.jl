...
function CenteredVerticalAdvection(spectral_grid; order = 2)
    CenteredVerticalAdvection{spectral_grid.NF, order ÷ 2}()
end
function UpwindVerticalAdvection(spectral_grid; order = 5)
    UpwindVerticalAdvection{spectral_grid.NF, (order + 1) ÷ 2}()
end
...
@inline retrieve_time_step(::DiffusiveVerticalAdvection, variables,
    var) = retrieve_previous_time_step(variables, var)
@inline retrieve_time_step(::DispersiveVerticalAdvection, variables,
    var) = retrieve_current_time_step(variables, var)
...
# 1st order upwind
@inline reconstructed_at_face(ξ, ij, k, u,
    ::UpwindVerticalAdvection{NF, 1}) where {NF} = ifelse(u > 0, ξ[ij, k[1]],
    ξ[ij, k[2]])

# 3rd order upwind
@inline reconstructed_at_face(ξ,
    ij,
    k,
    u,
    ::UpwindVerticalAdvection{NF, 2}) where {NF} = ifelse(
...
@inline reconstructed_at_face(ξ,
    ...
    k,
    u,
    ::UpwindVerticalAdvection{NF, 3}) where {NF} = ifelse(u > 0,
    (2ξ[ij, k[1]] - 13ξ[ij, k[2]] + 47ξ[ij, k[3]] + 27ξ[ij, k[4]] - 3ξ[ij, k[5]]) / 60,
    (2ξ[ij, k[6]] - 13ξ[ij, k[5]] + 47ξ[ij, k[4]] + 27ξ[ij, k[3]] - 3ξ[ij, k[2]]) / 60)

# 2nd order centered
@inline reconstructed_at_face(ξ, ij, k, u,
    ::CenteredVerticalAdvection{NF, 1}) where {NF} = (ξ[ij, k[1]] + ξ[ij, k[2]]) / 2
...
@inline reconstructed_at_face(ξ,
    ...
    k,
    u,
    ::CenteredVerticalAdvection{NF, 2}) where {NF} = (-ξ[ij, k[1]] + 7ξ[ij, k[2]] +
                                                      7ξ[ij, k[3]] - ξ[ij, k[4]]) / 12
...
@inline weight_β₀(S, NF) = NF(13/12) * (S[1] - 2S[2] + S[3])^2 +
                           NF(1/4) * (3S[1] - 4S[2] + S[3])^2
@inline weight_β₁(S, NF) = NF(13/12) * (S[1] - 2S[2] + S[3])^2 + NF(1/4) * (S[1] - S[3])^2
@inline weight_β₂(S, NF) = NF(13/12) * (S[1] - 2S[2] + S[3])^2 +
                           NF(1/4) * (S[1] - 4S[2] + 3S[3])^2

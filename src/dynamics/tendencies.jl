...
function pressure_gradient_flux!(
    ...
    transform!(∇lnp_x, ∇lnp_x_spec, diagn, S, unscale_coslat=true) # transform to grid: zonal gradient
    transform!(∇lnp_y, ∇lnp_y_spec, diagn, S, unscale_coslat=true) # meridional gradient
...
function surface_pressure_tendency!(
    ...
    transform!(pres_tend, pres_tend_grid, diagn, S)
...
function vordiv_tendencies!(
    ...
    transform!(u_tend, u_tend_grid, diagn, S)
    transform!(v_tend, v_tend_grid, diagn, S)
...
function physics_tendencies_only!(
    ...
    transform!(u_tend, u_tend_grid, diagn, S)
    transform!(v_tend, v_tend_grid, diagn, S)
    transform!(temp_tend, temp_tend_grid, diagn, S)
    model isa PrimitiveWet && transform!(humid_tend, humid_tend_grid, diagn, S)
...
function horizontal_advection!( 
    ...
    transform!(A_tend, A_tend_grid, diagn, S)  # for +A*div in spectral space
...
function flux_divergence!(
    ...
    transform!(uA, uA_grid, diagn, S)
    transform!(vA, vA_grid, diagn, S)
...
function bernoulli_potential!(
    ...
    transform!(bernoulli, bernoulli_grid, diagn, S)                # to spectral space
...
function SpeedyTransforms.transform!(
    ...
    transform!(vor_grid, vor, diagn, S)    # get vorticity on grid from spectral vor
    ...
    transform!(u_grid, U, diagn, S, unscale_coslat=true)
    transform!(v_grid, V, diagn, S, unscale_coslat=true)
 
    for (name, tracer) in model.tracers
        tracer.active && transform!(diagn.grid.tracers_grid[name], progn.tracers[name][lf], diagn, S)
    end
...
function SpeedyTransforms.transform!(
    ...
    transform!(vor_grid,  vor,  diagn, S)  # get vorticity on grid from spectral vor
    transform!(div_grid,  div,  diagn, S)  # get divergence on grid from spectral div
    transform!(pres_grid, pres, diagn, S)  # get η on grid from spectral η
...
function SpeedyTransforms.transform!(
    ...
    transform!(vor_grid,  vor,  diagn, S)  # get vorticity on grid from spectral vor
    transform!(div_grid,  div,  diagn, S)  # get divergence on grid from spectral div
    transform!(temp_grid, temp, diagn, S)  # -- temperature --
    transform!(pres_grid, pres, diagn, S)  # -- pressure --
    
    if model isa PrimitiveWet
        transform!(humid_grid, humid, diagn, S)        

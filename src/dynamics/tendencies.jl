...
function vertical_integration!(
    ...
    @inbounds for k in 1:nlayers    # integrate from top to bottom
        ...
        for ij in eachgridpoint(u_mean_grid, v_mean_grid, div_mean_grid)
            ...
            uv∇lnp_sum_above[ij, k] = u_mean_grid[ij]*∇lnp_x[ij] +
                                      v_mean_grid[ij]*∇lnp_y[ij]
    ...
    @inbounds for k in eachlayer(u_tend_grid, v_tend_grid)
        for (j, ring) in enumerate(rings)
            ...
            for ij in ring
                ...
                u_tend_grid[ij, k] = (u_tend_grid[ij, k] + v_grid[ij, k]*ω -
                                      RTᵥ*∇lnp_x[ij])*coslat⁻¹j
                v_tend_grid[ij, k] = (v_tend_grid[ij, k] - u_grid[ij, k]*ω -
                                      RTᵥ*∇lnp_y[ij])*coslat⁻¹j
            end
        end
    end
...
function temperature_tendency!(
    ...
    @inbounds for k in eachlayer(
        ...
        for ij in eachgridpoint(temp_tend_grid, temp_grid, div_grid, Tᵥ)
            temp_tend_grid[ij, k] += temp_grid[ij, k] * div_grid[ij, k] +            # +T'D term of hori advection
                                     κ * (Tᵥ[ij, k] + Tₖ) *
                                     (                          # +κTᵥ*Dlnp/Dt, adiabatic term
                                         σ_lnp_A *
                                         (div_sum_above[ij, k] + uv∇lnp_sum_above[ij, k]) +    # eq. 3.12 1st term
...
function tracer_advection!(
    ...
    for (name, tracer) in model.tracers
        ...
        tracer.active && horizontal_advection!(
            tracer_tend, tracer_tend_grid, tracer_grid, diagn, G, S, add = true)
    end
end
...
function SpeedyTransforms.transform!(
    ...
    for (name, tracer) in model.tracers
        ...
        tracer.active &&
            transform!(diagn.grid.tracers_grid[name], tracer_var, scratch_memory, S)
    end

...
function pressure_gradient_flux!(
    ...
    return @inbounds for k in eachlayer(u_grid, v_grid, uv∇lnp)
...
function temperature_anomaly!(
    ...
    return @inbounds for k in eachlayer(temp_grid, temp_virt_grid)
        Tₖ = temp_profile[k]
...
function vordiv_tendencies!(
    ...
    return vordiv_tendencies!(diagn, coriolis, atmosphere, geometry, spectral_transform)
end
...
function linear_pressure_gradient!(
    ...
    return @inbounds for k in eachmatrix(geopot)
...
function SpeedyTransforms.transform!(
    ...
    (;
        ...
        temp_grid_prev, humid_grid_prev, u_grid_prev, v_grid_prev,
    ) = diagn.grid

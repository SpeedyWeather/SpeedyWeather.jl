...
By default, a particle is of number format DEFAULT_NF and at 0˚N, 0˚E, σ=0.
$(TYPEDFIELDS)"""
struct Particle{
...
Particle{NF}(lon, lat) where NF = Particle{NF}(lon, lat, 0)
Particle{NF}(lon, lat, σ) where NF = Particle{NF}(lon, lat, σ)

# promotion of arguments if NF not provided
Particle(lon, lat) = Particle(lon, lat, 0)
Particle(lon, lat, σ) = Particle{promote_type(typeof.((lon, lat, σ))...)}(lon, lat, σ)
Particle(lon::Integer, lat::Integer) = Particle(lon,lat,0)
Particle(lon::Integer, lat::Integer, σ::Integer) = Particle{DEFAULT_NF}(lon, lat, σ)

# zero generators
Base.zero(::Type{Particle}) = Particle{DEFAULT_NF}(0,0,0)
Base.zero(::Type{Particle{NF}}) where NF = Particle{NF}(0,0,0)
...
function Base.:(==)(p1::Particle{NF1},p2::Particle{NF2}) where {NF1,NF2}
    return  (p1.lat == p2.lat) &&       # same latitude
...
function Base.show(io::IO,p::Particle{NF}) where {NF}
    ...
    σ = @sprintf("%.2f",p.σ)
    print(io,"Particle{$NF}($(lon)˚E, $(lat)˚N, σ = $σ)")
end
...
@inline function move(p::Particle{NF}, dlon, dlat, dσ) where NF
    (;lon, lat, σ) = p
    Particle{NF}(lon+dlon,lat+dlat,σ+dσ)
end
...
@inline function move(p::Particle{NF}, dlon, dlat) where NF
    (;lon, lat, σ) = p
    Particle{NF}(lon+dlon,lat+dlat,σ)
end

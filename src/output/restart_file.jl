...
@kwdef mutable struct RestartFile <: AbstractCallback
    ...
    pkg_version::VersionNumber = isnothing(pkgversion(SpeedyWeather)) ? v"0.0.0" :
                                 pkgversion(SpeedyWeather)
end
...
function finalize!(
    ...
    model.output.active ||
        @info "Restart file written to $(joinpath(path, filename)) although output=false"
end

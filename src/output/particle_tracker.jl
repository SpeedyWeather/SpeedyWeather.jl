...
function initialize!(
    ...
    mkpath(path)
    model.output.active ||
        @info "ParticleTracker writes to $(joinpath(path, filename)) as output=false"
    ...
    defVar(dataset, "lon", NF, ("particle", "time"),
        attrib =
    ...
    defVar(dataset, "lat", NF, ("particle", "time"),
        attrib =
        Dict("long_name"=>"latitude", "units"=>"degrees_east"),
        deflatelevel = particle_tracker.compression_level, shuffle = particle_tracker.shuffle)

    defVar(dataset, "sigma", NF, ("particle", "time"),
        attrib =
...
function finalize!(particle_tracker::ParticleTracker, args...)
    NCDatasets.close(particle_tracker.netcdf_file)
end

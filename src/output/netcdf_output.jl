...
function initialize!(
    ...
    defVar(dataset, "lon", lond, ("lon",),
        attrib = Dict("units"=>"degrees_east", "long_name"=>"longitude"))
    defVar(dataset, "lat", latd, ("lat",),
        attrib = Dict("units"=>"degrees_north", "long_name"=>"latitude"))
    defVar(dataset, "layer", σ, ("layer",),
        attrib = Dict("units"=>"1", "long_name"=>"sigma layer"))
    defVar(dataset, "soil_layer", soil_indices, ("soil_layer",),
        attrib = Dict("units"=>"1", "long_name"=>"soil layer index"))
...
function define_variable!(
    ...
    missing_value = hasfield(typeof(var), :missing_value) ? var.missing_value :
...
function get_indices(i, variable::AbstractOutputVariable)
    get_indices(i, Val.(variable.dims_xyzt)...)
end
...
function is_land(variable::AbstractOutputVariable)
    hasproperty(variable, :is_land) ? variable.is_land : false
end
...
function output!(
    ...
    var = is3D(variable) ? (is_land(variable) ? output.field3Dland : output.field3D) :
          output.field2D
...
function get_full_output_file_path(output::AbstractOutput)
    joinpath(output.run_path, output.filename)
end

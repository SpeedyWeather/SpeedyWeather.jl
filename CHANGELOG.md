...
- Remove dead code: unused `vorticity_flux!`/`vorticity_flux_curldiv!`, `quadrant_ring`, `ring_order!`/`nested_order!`/`matrix_order!`, `get_2lm_range`, `AllOutputVariables`, and the `BernoulliPotential` dummy type [#????](https://github.com/SpeedyWeather/SpeedyWeather.jl/pull/????)
